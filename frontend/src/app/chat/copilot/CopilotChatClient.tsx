...
import { useCopilotChat as useCopilotChatInternal, type UseCopilotChatReturn as UseCopilotChatReturn_c } from "@copilotkit/react-core";
...
function useSessionState(initial?: string, defaultAgent: 'primary' | 'log_analysis' = 'primary') {
  ...
  const ensureSession = useCallback(async () => {
    ...
    lastCreatedRef.current = createdId;
    try {
...
function SimpleChatUI({ apiPath, initialSessionId, agentType }: { apiPath?: string; initialSessionId?: string; agentType?: 'primary' | 'log_analysis' }) {
  ...
  const resolvedApiPath = useMemo(() => {
    ...
    return `${normalized}/api/v1/copilot/stream`;
  }, [apiPath]);
  ...
  return (
    <CopilotKit
      ...
      headers={headers}
      credentials="include"
      // Forward per-request properties (session and memory flag)
      properties={{
        session_id: sessionId,
        use_server_memory: memoryEnabled,
        ...requestProps,
        ...(mcpUrl ? { mcpEndpoints: [{ endpoint: mcpUrl }] } : {}),
      }}
...
function ChatInner({
  ...
  const chat: UseCopilotChatReturn_c = useCopilotChatInternal({});
  ...
  const [interruptQueue, setInterruptQueue] = useState<Array<{ event: any; resolve: (value: any) => void }>>([]);
  ...
  // Listen for external follow-up triggers (e.g., FeedMe → Chat bridge)
  useEffect(() => {
    const handler = (e: Event) => {
      try {
        const ce = e as CustomEvent<{ question?: string }>;
        const q = ce?.detail?.question;
        if (typeof q === 'string') setInput(q);
      } catch {}
    };
    if (typeof window !== 'undefined') {
  ...
  const onFilesPicked = useCallback(async (files: FileList | null) => {
    ...
    const allowedTypes = new Set<string>([
      ...UI_CONFIG.VALIDATION.ALLOWED_FILE_TYPES,
      'text/plain',
      'text/markdown',
      'text/csv',
      'text/html',
  ...
  const send = useCallback(async () => {
    ...
    if (text && handleSlashCommand(text)) { setInput(''); return; }
    setIsSending(true);
    try {
      const sid = await ensureSession();
      const base: any = { id: uuidv4(), role: 'user', content: text || '' };
      let messageToSend: any = base;
      ...
      if (attachments.length > 0 && attachments[0].type.startsWith('image/')) {
        ...
        if (img) (messageToSend as any).image = img;
      }
      const traceId = uuidv4();
      const resolvedAgent = agentType || 'primary';
      flushSync(() => setRequestProps((prev) => ({
        ...prev,
      ...
      if (typeof (chat as any).sendMessage === 'function') {
        await (chat as any).sendMessage(messageToSend);
      } else if (typeof (chat as any).appendMessage === 'function') {
        // Fallback to legacy API: append a GQL TextMessage for user content
        await (chat as any).appendMessage(new TextMessage({ role: GqlRole.User, content: text || '' }));
      } else {
        throw new Error('Copilot chat API unavailable: neither sendMessage nor appendMessage present');
      }
      await sessionsAPI.postMessage(sid, { message_type: 'user', agent_type: resolvedAgent, content: text || (attachments.length ? `[${attachments.length} image(s) attached]` : '') });
    } finally {
      setInput("");
      setAttachments([]);
      flushSync(() => setRequestProps((prev) => { const next = { ...prev }; delete next.attachments; delete next.trace_id; return next; }));
      setIsSending(false);
    }
  }, [input, attachments, chat, ensureSession, handleSlashCommand, setRequestProps, provider, model, agentType]);
...
useEffect(() => {
    (async () => {
      try {
        const sid = sessionId;
        if (!sid) return;
        const last = [...(chat.messages || [])].reverse().find((m) => (m as any).role === 'assistant' && typeof (m as any).content === 'string');
        if (!last) return;
...
}, [chat.messages?.length, sessionId, agentType]);
    ...
    <>
      <div className="max-w-3xl w-full mx-auto space-y-4">
        <div className="flex items-center justify-between gap-4">
          <h2 className="text-xl font-semibold">CopilotKit Chat</h2>
          <div className="flex items-center gap-4 ml-auto">
            <label className="inline-flex items-center gap-2 text-sm">
              <input type="checkbox" aria-label="Use server memory" className="accent-primary" checked={memoryEnabled} onChange={(e) => setMemoryEnabled(e.target.checked)} />
              Use server memory
            </label>
            <ModelSelector
              provider={provider}
        ...
        <div className="rounded-lg border divide-y bg-background/60">
          ...
          <form className="flex items-center gap-2 p-3" onSubmit={(e) => { e.preventDefault(); if (!isSending) send(); }}>
            <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Type a message..." className="flex-1 px-3 py-2 border rounded-md bg-background" />
            ...
            <button type="submit" disabled={isSending || (!input.trim() && attachments.length === 0)} className="px-4 py-2 rounded-md bg-primary text-primary-foreground disabled:opacity-50">
          ...
          {attachments.length > 0 && (
            <div className="flex flex-wrap gap-2 p-3">
              {attachments.map((f, i) => {
                ...
                return (
                  <a key={i} href={url} download={f.name || 'attachment'} className="text-xs rounded-full px-3 py-1 border bg-muted/30 hover:bg-muted/50 transition-colors" onClick={(e) => { if (!url) e.preventDefault(); }} title={f.type ? `${f.name} • ${f.type}` : f.name}>
                    {f.name}
                  </a>
                );
              })}
      ...
      {interruptQueue.length > 0 && (() => {
        ...
        const details = (() => { try { return JSON.stringify(current?.event?.value ?? {}, null, 2); } catch { return undefined; } })();
        const resolveAndPop = (decision: 'approve' | 'reject') => { try { current.resolve(decision); } catch {} setInterruptQueue((q) => q.slice(1)); };
        return (
          <Dialog open>
            <DialogContent hideClose>
              ...
              <DialogFooter className="mt-4">
                <UIButton onClick={() => resolveAndPop('reject')} variant="secondary">Reject</UIButton>
      ...
      <FeedbackDialog open={feedbackOpen} initialFeedback={feedbackText} selectedText={feedbackSelected} sessionId={sessionId || null} agent={agentType} onClose={() => setFeedbackOpen(false)} />
      <CorrectionDialog open={correctionOpen} initialIncorrect={incorrectText} initialCorrected={correctedText} sessionId={sessionId || null} agent={agentType} onClose={() => setCorrectionOpen(false)} />
    </>
  );
}
