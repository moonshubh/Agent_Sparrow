...
export function ChatHistorySidebar({ sessionId, onSelect }: Props) {
    ...
    async (
      ...
      try {
        ...
        if (silent) {
          ...
          const message = refreshError instanceof Error ? refreshError.message : 'Failed to load sessions'
          setError(message)
        }
      } finally {
        if (!silent && !isAborted()) {
  ...
  useEffect(() => {
    ...
    renameTimerRef.current = timeout
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              ...
              {sessions.map((s) => {
                ...
                const errorCount = typeof logMetadata?.error_count === 'number'
                  ? logMetadata.error_count
                  : 0
                const warningCount = typeof logMetadata?.warning_count === 'number'
                  ? logMetadata.warning_count
                  : undefined
                const healthStatus = typeof logMetadata?.health_status === 'string'
                                  ...
                                  {isLogAnalysis && logMetadata && (
                                    <>
                                      ...
                                      {warningCount !== undefined && (
                                        <div className="text-yellow-500">Warnings: {warningCount}</div>
                                      )}
                                      {healthStatus && (
