...
interface AnalyticsActions {
  ...
  getTopPerformingFolders: (
    limit?: number,
  ) => Array<{ folder_id: number; score: number }>;
}
      ...
      actions: {
        ...
        loadWorkflowStats: async (forceRefresh = false) => {
          ...
          try {
            ...
            console.warn(
              "Analytics: service unreachable; skipping workflow stats load",
            );
        ...
        loadPerformanceMetrics: async (forceRefresh = false) => {
          if (!forceRefresh && get().performanceMetrics) {
            return;
          }
          ...
          try {
            ...
            console.warn(
              "Analytics: failed to load performance metrics (mock used)",
            );
        ...
        loadUsageStats: async (forceRefresh = false) => {
          if (!forceRefresh && get().usageStats) {
            return;
          }
          ...
          try {
            ...
            const stats: UsageStats = {
              ...
              most_active_folders: [
                {
                  folder_id: 1,
                  folder_name: "Technical Support",
                  activity_count: 234,
                },
                {
                  folder_id: 2,
                  folder_name: "Account Issues",
                  activity_count: 189,
                },
                {
                  folder_id: 3,
                  folder_name: "Feature Requests",
                  activity_count: 156,
                },
              ],
              trending_tags: [
        ...
        loadSystemMetrics: async (forceRefresh = false) => {
          if (!forceRefresh && get().systemMetrics) {
            return;
          }
        ...
        loadQualityMetrics: async (forceRefresh = false) => {
          if (!forceRefresh && get().qualityMetrics) {
            return;
          }
          ...
          if (format === "json") {
            ...
            const csvData = [
              ...
              ...(state.performanceMetrics
                ? Object.entries(state.performanceMetrics).map(
                    ([key, value]) => [key, value, "Performance"],
                  )
                : []),
              // Usage stats
              ...(state.usageStats
                ? Object.entries(state.usageStats)
                    .filter(([, value]) => typeof value === "number")
                    .map(([key, value]) => [key, value, "Usage"])
                : []),
              // System metrics
              ...(state.systemMetrics
              ...
              ...(state.qualityMetrics
                ? Object.entries(state.qualityMetrics).map(([key, value]) => [
                    key,
                    value,
                    "Quality",
                  ])
...
const selectPerformanceMetrics = (state: AnalyticsStore) =>
  state.performanceMetrics;
