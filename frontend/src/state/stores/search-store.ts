          ...
          actions: {
              ...
              try {
                ...
                const searchRequest: SearchExamplesRequest = {
                  ...
                  filters: {
                    ...
                    // Other filters
                    folder_ids:
                      state.filters.folders.length > 0
                    ...
                    issue_types:
                      state.filters.issueTypes.length > 0
                        ? state.filters.issueTypes
                        : undefined,
                    resolution_types:
                      state.filters.resolutionTypes.length > 0
                        ? state.filters.resolutionTypes
                        : undefined,
                  },
                ...
                const response: SearchExamplesResponse =
                  await feedMeApi.searchExamples(searchRequest);
                ...
                // Record analytics
                get().actions.recordSearchAnalytics(
                  query,
                  responseTime,
            ...
            addToHistory: (query, resultsCount) => {
              ...
              set((state) => {
                ...
                const filteredRecent = state.recentSearches.filter(
                  (r) => r.query !== trimmedQuery,
                );

                return {
                  searchHistory: [trimmedQuery, ...filteredHistory].slice(
                    0,
                    50,
                  ),
                  recentSearches: [
            ...
            recordSearchAnalytics: (query, responseTime, resultCount) => {
              set((state) => {
                ...
                const existingQuery = analytics.popular_queries.find(
                  (p) => p.query === query,
                );
                if (existingQuery) {
                  existingQuery.count++;
                ...
                if (
                  resultCount === 0 &&
                  !analytics.zero_result_queries.includes(query)
                ) {
            ...
            parseSearchUrl: (url) => {
              ...
              const folders =
                params.get("folders")?.split(",").map(Number) || [];
