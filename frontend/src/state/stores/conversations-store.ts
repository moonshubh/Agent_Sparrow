...
const normalizeConversation = (
  ...
  const updatedTimestamp = conv.last_updated ?? (conv as { updated_at?: string }).updated_at ?? conv.created_at ?? null
...
): request is { title: string } => 'title' in request && Object.keys(request).length === 1
...
interface ConversationsActions {
  ...
  editConversation: (id: number, request: ConversationEditRequest | { title: string }) => Promise<void>
      ...
      actions: {
        ...
        loadConversations: async (options = {}) => {
          ...
          try {
            ...
            const normalizedConversations = response.conversations.map(conv =>
              normalizeConversation(conv as UploadTranscriptResponse & Partial<Conversation>)
            )
        ...
        getConversation: async (id, forceRefresh = false) => {
          ...
          const loadPromise = (async () => {
            try {
              ...
              console.warn(`Conversation ${id}: service unreachable; returning null`, error)
        ...
        uploadConversation: async (uploadData) => {
          ...
          try {
            ...
            const conversation = normalizeConversation(response as UploadTranscriptResponse & Partial<Conversation>)
            set(state => ({
              conversations: {
                ...state.conversations,
        ...
        approveConversation: async (id, request) => {
          ...
          const supabaseRequest = {
            ...
            if (conversation?.folder_id) {
              ...
              import('@/state/stores/folders-store').then(({ useFoldersStore }) => {
        ...
        editConversation: async (id, request) => {
          try {
            ...
            if (isTitleOnlyUpdate(request)) {
              ...
              const normalized = normalizeConversation(result.conversation as UploadTranscriptResponse & Partial<Conversation>)
              get().actions.updateConversation(id, normalized)
            }
        ...
        bulkAssignToFolder: async (ids, folderId) => {
          ...
          try {
            ...
            const { assignConversationsToFolderSupabase } = await import('@/features/feedme/services/feedme-api')
        ...
        deleteExample: async (exampleId) => {
          ...
          try {
            ...
            const { deleteExample } = await import('@/features/feedme/services/feedme-api')
        ...
        getConversationStats: () => {
          ...
          return {
            ...
            approved: conversations.filter(c => c.approval_status === 'approved').length,
            pending: conversations.filter(c => c.approval_status !== 'approved' && c.approval_status !== 'rejected').length,
            rejected: conversations.filter(c => c.approval_status === 'rejected').length,
