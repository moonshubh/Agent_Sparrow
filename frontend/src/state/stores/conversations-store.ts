...
import {
  ...
  type FeedMeExample as ApiFeedMeExample,
...
function isApiError(error: unknown): error is ApiError {
  ...
  return typeof maybeStatusText === "string";
}
...
// Types
export type FeedMeExample = ApiFeedMeExample & {
  uuid?: string;
...
const normalizeConversation = (
  conv: UploadTranscriptResponse & Partial<Conversation>,
): Conversation => {
  const id = conv.id ?? conv.conversation_id;
  const folderId = (conv as { folder_id?: number | null }).folder_id ?? null;
  const updatedTimestamp =
    conv.last_updated ??
    (conv as { updated_at?: string }).updated_at ??
    conv.created_at ??
    null;
  ...
  return {
    ...
    id,
    folder_id: folderId,
    last_updated: updatedTimestamp ?? undefined,
  };
};

const isTitleOnlyUpdate = (
  request: ConversationEditRequest | { title: string },
): request is { title: string } =>
  "title" in request && Object.keys(request).length === 1;
...
interface ConversationsActions {
  ...
  // Upload Operations
  uploadConversation: (
    upload: Omit<ConversationUpload, "id" | "status" | "progress">,
  ) => Promise<string>;
  ...
  bulkApprove: (
    ids: number[],
    request: Omit<ApprovalRequest, "conversation_id">,
  ) => Promise<void>;
  bulkReject: (
    ids: number[],
    request: Omit<RejectionRequest, "conversation_id">,
  ) => Promise<void>;
      ...
      actions: {
          ...
          try {
            ...
            const normalizedConversations = response.conversations.map((conv) =>
              normalizeConversation(
                conv as UploadTranscriptResponse & Partial<Conversation>,
              ),
            );
            ...
            console.warn(
              "Conversations: service unreachable; using empty list",
            );
            ...
            if (isNotFoundError(error)) {
              ...
              console.warn(
                `Folder ${folderId} not found, reset to show all conversations`,
              );
            } else {
              set((state) => ({
                conversationsList: {
        ...
        setCurrentFolder: async (folderId) => {
          ...
          try {
            ...
            console.error(
              "Failed to load conversations for folder:",
              folderId,
              error,
            );
        ...
        getConversation: async (id, forceRefresh = false) => {
          ...
          const loadPromise = (async () => {
            try {
              ...
              const response = await fetch(
                `/api/v1/feedme/conversations/${id}`,
              );
              ...
              console.warn(
                `Conversation ${id}: service unreachable; returning null`,
                error,
              );
          ...
          set((state) => ({
            _internal: {
              ...state._internal,
              activeRequests: new Map(state._internal.activeRequests).set(
                loadingKey,
                loadPromise,
              ),
            },
        ...
        removeConversation: (id) => {
          set((state) => {
            ...
            return {
              ...
              conversationsList: {
                ...state.conversationsList,
                items: state.conversationsList.items.filter(
                  (item) => item.id !== id,
                ),
        ...
        deleteMultipleConversations: async (ids) => {
          ...
          if (errors.length > 0) {
            const errorDetails = errors
              .map(({ id, error }) => `Conversation ${id}: ${error.message}`)
              .join("; ");
        ...
        uploadConversation: async (uploadData) => {
          ...
          try {
            ...
            if (upload.type === "file" && upload.file) {
              ...
              throw new Error(
                "Invalid upload configuration: only PDF file uploads are supported",
              );
            }
            ...
            const conversation = normalizeConversation(
              response as UploadTranscriptResponse & Partial<Conversation>,
            );
            set((state) => ({
              conversations: {
                ...state.conversations,
            ...
            if (upload.autoProcess) {
              set((state) => ({
                processing: {
                  ...state.processing,
                  processingQueue: [
                    ...state.processing.processingQueue,
                    conversation.id,
                  ],
                  processingStatus: {
        ...
        reprocessConversation: async (id) => {
          try {
            ...
            set((state) => ({
              processing: {
                ...state.processing,
                processingQueue: state.processing.processingQueue.filter(
                  (queueId) => queueId !== id,
                ),
                processingStatus: {
                  ...state.processing.processingStatus,
        ...
        reprocessMultipleConversations: async (ids) => {
          ...
          if (errors.length > 0) {
            ...
            throw new Error(
              `Failed to reprocess ${errors.length} conversations`,
            );
          }
        },
        ...
        approveConversation: async (id, request) => {
          ...
          try {
            ...
            const response = await fetch(
              `/api/v1/feedme/conversations/${id}/examples/approve`,
              {
                method: "POST",
                headers: {
            ...
            showToast({
              ...
              message:
                data.message ||
                `Successfully approved ${data.approved_count} examples. Syncing to Supabase...`,
              type: "success",
              duration: 5000,
            });
            ...
            if (conversation?.folder_id) {
              ...
              import("@/state/stores/folders-store").then(
                ({ useFoldersStore }) => {
        ...
        editConversation: async (id, request) => {
          try {
            ...
            if (isTitleOnlyUpdate(request)) {
              const result = await apiUpdateConversation(id, {
                title: request.title,
              });
              const normalized = normalizeConversation({
                ...result,
                conversation_id: result.id,
              } as UploadTranscriptResponse & Partial<Conversation>);
              get().actions.updateConversation(id, normalized);
            } else {
              const result = await apiEditConversation(id, request);
              const normalized = normalizeConversation(
                result.conversation as UploadTranscriptResponse &
          ...
          set((state) => {
            ...
            if (useShift && state.selection.lastSelectedId !== null) {
              ...
              const lastIndex = conversations.findIndex(
                (c) => c.id === state.selection.lastSelectedId,
              );
        ...
        bulkAssignToFolder: async (ids, folderId) => {
          ...
          try {
            ...
            const { assignConversationsToFolderSupabase } =
              await import("@/features/feedme/services/feedme-api");

            // Make the API call with Supabase sync
            const result = await assignConversationsToFolderSupabase(
              folderId,
              ids,
            );

            console.log(
              `Successfully assigned ${ids.length} conversations to folder ${folderId}`,
            );
              ...
              set((state) => {
                ...
                return {
                  ...
                  conversationsList: {
                    ...
                    totalCount: Math.max(
                      0,
                      state.conversationsList.totalCount - ids.length,
                    ),
                  },
                };
              });
            ...
            useUIStore.getState().actions.showToast({
              ...
              message:
                result.message ||
                `Assigned ${ids.length} conversations to folder`,
              duration: 3000,
            });
          } catch (error) {
        ...
        loadExamples: async (conversationId, forceRefresh = false) => {
          ...
          // Check cache
          const lastUpdated =
            state.examples.examplesLastUpdated[conversationId];
          const cacheValid =
            lastUpdated &&
          ...
          if (
            !forceRefresh &&
            cacheValid &&
            state.examples.examplesByConversation[conversationId]
          ) {
            return;
          }
          ...
          try {
            const response =
              await feedMeApi.getConversationExamples(conversationId);
            ...
            if (
              ...
              console.warn(
                `Conversation ${conversationId} no longer exists, removing from state`,
              );
            ...
            console.error(
              `Failed to load examples for conversation ${conversationId}:`,
              error,
            );
        ...
        updateExample: async (exampleId, updates) => {
          ...
          for (const [convId, examples] of Object.entries(
            state.examples.examplesByConversation,
          )) {
          ...
          set((state) => ({
            examples: {
              ...
              examplesByConversation: {
                ...state.examples.examplesByConversation,
                [conversationId!]: state.examples.examplesByConversation[
                  conversationId!
                ].map((ex) => (ex.id === exampleId ? optimisticExample : ex)),
              },
              optimisticUpdates: {
          ...
          try {
            ...
            if (state.conversations[conversationId]) {
              const currentExamples =
                get().examples.examplesByConversation[conversationId] || [];
              set((state) => ({
                conversations: {
                  ...state.conversations,
                  [conversationId!]: {
          ...
          if (examples.length === 0) {
            ...
            try {
              ...
              const updatedExamples =
                updatedState.examples.examplesByConversation[conversationId] ||
                [];
              ...
              if (updatedExamples.length > 0) {
                ...
                useUIStore.getState().actions.showToast({
                  ...
                  message:
                    "This conversation does not contain extractable Q&A examples",
                  duration: 4000,
                });
              }
            } catch (error) {
              ...
              useUIStore.getState().actions.showToast({
                ...
                message:
                  "Could not reprocess the conversation. Please try again.",
                duration: 5000,
              });
            }
          }
...
export const useConversations = () =>
  useConversationsStore((state) => state.conversationsList);

export const useConversationSelection = () =>
  useConversationsStore((state) => state.selection);

export const useProcessingState = () =>
  useConversationsStore((state) => state.processing);

export const useConversationsActions = () =>
  useConversationsStore((state) => state.actions);

export const useConversationById = (id: number) =>
  useConversationsStore((state) => state.conversations[id]);
