    ...
    if (callbacks) {
      callbacks.forEach((callback) => {
        try {
          ...
          console.error(
            `Error in store event handler for ${event.type}:`,
            error,
          );
        }
      });
...
export function useStoreSync() {
  ...
  useEffect(() => {
    ...
    const unsubscribeProcessing = storeEventBus.subscribe(
      "processing_completed",
      (event) => {
        if (event.type === "processing_completed") {
          const { processing_status: _ignoredStatus, ...restResult } =
    ...
    const unsubscribeFolderUpdates = storeEventBus.subscribe(
      "folder_updated",
      (event) => {
    ...
    const unsubscribeSearch = storeEventBus.subscribe(
      "search_performed",
      (event) => {
    ...
    const unsubscribeWsConnect = storeEventBus.subscribe(
      "websocket_connected",
      () => {
    ...
    const unsubscribeWsDisconnect = storeEventBus.subscribe(
...
export function useConversationManagement() {
  ...
  const deleteConversationWithConfirmation = useCallback(
    async (id: number) => {
  ...
  const uploadWithProgress = useCallback(
    async (title: string, file: File, folderId?: number) => {
      try {
        ...
        if (folderId) {
          try {
            ...
            const unsubscribe = storeEventBus.subscribe(
              "conversation_uploaded",
              (event) => {
                if (
                  event.type === "conversation_uploaded" &&
                  ...
                  foldersActions
                    ...
                    .then(() => {
                      ...
                      uiActions.showToast({
                        ...
                        message:
                          "The conversation has been assigned to the selected folder.",
                      });
                    })
                    .catch((error) => {
                      uiActions.showToast({
                        ...
                        message:
                          "The conversation was uploaded but could not be assigned to the folder.",
                      });
                    })
                    .finally(() => {
                      unsubscribe();
...
export function useFolderManagement() {
  ...
  const createFolderWithValidation = useCallback(
    async (name: string, parentId?: number, description?: string) => {
      if (!name.trim()) {
        uiActions.showToast({
          type: "error",
          title: "Invalid Name",
  ...
  const moveConversationsToFolder = useCallback(
    async (conversationIds: number[], targetFolderId: number | null) => {
      try {
        uiActions.setOperationLoading("move-conversations", true);

        await foldersActions.assignConversationsToFolder(
          conversationIds,
          targetFolderId,
        );
...
export function useSearchIntegration() {
  ...
  const performSearchWithAnalytics = useCallback(
    async (
      query: string,
...
export function useBulkOperations() {
  ...
  const selectedConversationIds = useMemo(() => {
    return Array.from(ui.selectedItems).filter(
      (id) => typeof id === "number",
    ) as number[];
  }, [ui.selectedItems]);
      ...
      uiActions.openModal("bulkDeleteConfirmation", {
        ...
        onConfirm: async () => {
          try {
            ...
            await conversationsActions.deleteMultipleConversations(
