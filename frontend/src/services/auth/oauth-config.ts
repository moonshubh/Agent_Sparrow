...
const isOAuthGloballyEnabled = parseEnvBoolean(
...
export const oauthConfig: OAuthConfig = Object.freeze({
  google: Object.freeze({
    ...
    redirectUri:
      process.env.NEXT_PUBLIC_AUTH_REDIRECT_URL || getDefaultRedirectUri(),
    scope: "openid email profile",
    enabled:
      isOAuthGloballyEnabled && !!process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID,
  }),
  github: Object.freeze({
    ...
    scope: "user:email",
    enabled:
      isOAuthGloballyEnabled && !!process.env.NEXT_PUBLIC_GITHUB_CLIENT_ID,
  }),
});

export const isOAuthEnabled = isOAuthGloballyEnabled;
export const isDebugAuthEnabled = parseEnvBoolean(
  process.env.NEXT_PUBLIC_ENABLE_DEBUG_AUTH,
);
