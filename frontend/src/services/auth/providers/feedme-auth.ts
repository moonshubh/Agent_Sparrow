...
class FeedMeAuth {
  ...
  login(
    userId: string,
    role: "admin" | "moderator" | "viewer" | "user" = "user",
  ): boolean {
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        if (storedUser && storedToken) {
          ...
          if (this.isTokenExpired(storedToken)) {
            ...
            console.log(
              `✓ FeedMe Auth: Restored session for ${this.currentUser?.email}`,
            );
          }
        }
      }
...
export const hasPermission = (permission: string) =>
  feedMeAuth.hasPermission(permission);
export const login = (
  userId: string,
  role: "admin" | "moderator" | "viewer" | "user" = "user",
) => feedMeAuth.login(userId, role);
...
export const getWebSocketUrl = (baseUrl: string) =>
  feedMeAuth.getWebSocketUrl(baseUrl);
