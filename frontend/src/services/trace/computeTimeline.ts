import { TimelineInput, TimelineStep } from '@/shared/types/trace';
...
export function computeTimeline(input: TimelineInput): TimelineStep[] {
  ...
  if (thinking?.thinking_steps && Array.isArray(thinking.thinking_steps)) {
    for (const st of thinking.thinking_steps) {
      const title =
        PHASE_TITLES[st.phase] ||
        st.phase?.toString()?.replaceAll('_', ' ').toLowerCase().replace(/\b\w/g, (c: string) => c.toUpperCase()) ||
        'Step';
