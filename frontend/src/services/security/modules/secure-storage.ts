  ...
  private async initializeKey(): Promise<void> {
    ...
    try {
      ...
      const keyData = encoder.encode(
        this.keyString.substring(0, 32).padEnd(32, "0"),
      );
  ...
  async decrypt(data: string): Promise<string> {
    ...
    if (
      ...
      try {
        ...
        logger.warn("Secure storage decrypt failed, attempting fallback", {
          error,
        });
...
export class SecureStorage {
  ...
  async set<T>(
    key: string,
    value: T,
    options: { expiry?: number; encrypt?: boolean } = {},
  ): Promise<void> {
  ...
  keys(): string[] {
    if (this.config.type === StorageType.MEMORY) {
      return Array.from(this.memoryStorage.keys()).map((k) =>
  ...
  private sanitizeString(str: string): string {
    ...
    return (
      str
        ...
        .replace(
          /<(iframe|object|embed|applet|meta|link|style|base|form)\b[^>]*>/gi,
          "",
        )
  ...
  useEffect(() => {
    const loadValue = async () => {
      try {
        ...
        logger.error("Failed to load secure storage value", {
          error: err,
          key,
        });
      } finally {
