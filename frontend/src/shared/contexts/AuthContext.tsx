...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
      ...
      try {
        ...
        if (localAuthBypass || (bypassAuth && devMode)) {
          ...
          if (localAuthBypass) {
            try {
              ...
              console.warn(
                "Local auth token init failed (continuing in UI-only bypass):",
                e,
              );
            }
          }
          ...
          if (localUser) {
            try {
              ...
              const mockUser: User = {
                ...
                last_sign_in_at:
                  parsedUser.last_sign_in_at || new Date().toISOString(),
        ...
        try {
          ...
          if (error) {
            ...
            console.log(
              "AuthContext: Found session, setting user:",
              session.user.email,
            );
  ...
  const loginWithOAuth = useCallback(async (provider: "google" | "github") => {
    try {
      ...
      const allowedDomain = (
        process.env.NEXT_PUBLIC_ALLOWED_EMAIL_DOMAIN || "getmailbird.com"
      ).trim();
