...
import { FEATURE_FLAGS, APP_CONFIG } from "@/shared/config/constants";
...
class ConsoleTransport implements LogTransport {
  ...
  private logBrowser(entry: LogEntry): void {
    ...
    console.group(`%c[${timestamp}] [${level}] ${entry.message}`, style);
        ...
        if (this.buffer.length > 0) {
          ...
          if (navigator.sendBeacon) {
            ...
            try {
              ...
              console.error(
                "[RemoteTransport] Failed to send logs on unload:",
                error,
              );
            }
          }
...
class Logger {
  ...
  private initialize(): void {
    ...
    if (
      FEATURE_FLAGS.ENABLE_ERROR_REPORTING &&
