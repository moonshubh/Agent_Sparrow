  ...
  wrap<TArgs extends unknown[], TResult>(
    ...
    return async (...args: TArgs) => {
      try {
        ...
        await this.handle(error instanceof Error ? error : new Error(String(error)), { silent: options.silent })
        throw error
      }
    }
  }
...
export function useErrorHandler(config?: ErrorHandlerConfig) {
  ...
  const wrapAsync = useCallback(<TArgs extends unknown[], TResult>(
    fn: (...args: TArgs) => Promise<TResult>,
    options?: { name?: string; silent?: boolean }
  ) => {
  ...
  const safe = useCallback(<TArgs extends unknown[], TResult>(
    fn: (...args: TArgs) => Promise<TResult> | TResult
  ) => {
