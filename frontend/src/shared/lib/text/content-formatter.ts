  ...
  instructionPatterns.forEach((pattern) => {
    content = content.replace(pattern, (match, intro, instructions) => {
      ...
      const stepIndicators =
        /\. (In |Click |Hold |Open |Please |Make |This |Now |Then |Next |Finally |After )/gi;
      const steps: string[] = instructions
        .split(stepIndicators)
...
function highlightTechnicalElements(content: string): string {
  ...
  content = content.replace(
    /\b([A-Za-z0-9_-]+\.(log|txt|png|jpg|jpeg|pdf|zip|exe|msi))\b/gi,
    "`$1`",
  );
  ...
  content = content.replace(
    /\b(CTRL|ALT|SHIFT|CMD)\s*\+\s*([A-Z0-9]+)/gi,
    "`$1+$2`",
  );
...
function formatSupportPatterns(content: string): string {
  // Greeting patterns
  content = content.replace(/^(Hello [^,]+),?\s*/, "**$1**\n\n");
