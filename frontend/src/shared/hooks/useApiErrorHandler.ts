...
export function useApiErrorHandler(options: ApiErrorHandlerOptions = {}) {
  ...
  const defaultShouldRetry = useCallback(
    (error: NormalizedError, attemptNumber: number): boolean => {
  ...
  const effectiveShouldRetry = useCallback(
    (error: NormalizedError, attemptNumber: number) => {
      return shouldRetry
        ? shouldRetry(error, attemptNumber)
        : defaultShouldRetry(error, attemptNumber);
    },
    ...
    (rawError: unknown, customMessage?: string) => {
      ...
      if (error instanceof ApiUnreachableError) {
        switch (error.errorType) {
          case "timeout":
            message =
              "The request is taking longer than expected. The server might be busy.";
            toastType = "warning";
            break;
          case "network":
