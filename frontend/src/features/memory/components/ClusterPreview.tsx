...
function ClusterPreviewContent({
  ...
  const [preview, setPreview] =
    useState<SplitRelationshipPreviewResponse | null>(null);
  ...
  const entityLabelById = useMemo(() => {
    const getLabel = (id: string) =>
      nodeById.get(id)?.node.displayLabel ??
      nodeById.get(id)?.node.entityName ??
      id;
    return { getLabel };
  }, [nodeById]);
    ...
    previewMutation.mutate(
      ...
      },
      {
        ...
        onError: (err: unknown) => {
          if (requestIdRef.current !== requestId) return;
          setError(
            err instanceof Error ? err.message : "Failed to load split preview",
          );
        },
      },
    );
  ...
  const existingRelationshipCount =
    preview?.existing_relationship_ids?.length ?? 0;
  ...
  const handleCommit = async () => {
    setError(null);
    const payloadClusters: SplitRelationshipCommitCluster[] = clusters.map(
      (c) => ({
        name: c.name,
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to commit relationship split",
      );
    }
  };
      ...
      >
        ...
        <div className="cluster-preview__meta">
          <div className="cluster-preview__meta-line">
            Replaces <strong>{existingRelationshipCount}</strong> existing
            relationship
        ...
        {previewMutation.isPending ? (
          ...
          <div className="cluster-preview__list">
            {clusters.map((cluster) => {
              const sourceLabel = entityLabelById.getLabel(
                cluster.source_entity_id,
              );
              const targetLabel = entityLabelById.getLabel(
                cluster.target_entity_id,
              );
                ...
                <div key={cluster.cluster_id} className="cluster-preview__card">
                  ...
                  <div className="cluster-preview__row">
                    ...
                    <div className="cluster-preview__direction">
                      <span
                        className="cluster-preview__direction-label"
                        title={sourceLabel}
                      >
                        {sourceLabel}
                      </span>
