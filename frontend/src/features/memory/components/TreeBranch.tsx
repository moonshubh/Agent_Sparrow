...
export function TreeBranch({
  ...
  const hitGeometry = useMemo(
    () => new THREE.TubeGeometry(curve, 18, Math.max(radius + 0.12, 0.18), 6, false),
    [curve, radius]
  );
  ...
  }, [barkTextures.map, barkTextures.normalMap, baseColor, isDimmed, isGhosted, link.gapReason, viewMode]);
  ...
  useFrame((state) => {
    ...
    if (viewMode === 'celebrate_strengths') {
      ...
      const intensity = link.strength === 'strong'
        ? 0.95
        : link.strength === 'medium'
          ? 0.52
          : 0.08;
  ...
  return (
    <group>
      ...
      />
      {glowGeometry ? <mesh geometry={glowGeometry} material={glowMaterial} /> : null}
      {showVeins ? (
        <line ref={veinRef} geometry={veinGeometry} material={veinMaterial} />
      ) : null}
      {edge && (
        <mesh
