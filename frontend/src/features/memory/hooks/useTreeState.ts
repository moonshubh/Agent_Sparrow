...
function parseTreeStateV2(raw: unknown): PersistedTreeStateV2 | null {
  ...
  const showAllLabels = typeof raw.showAllLabels === 'boolean' ? raw.showAllLabels : false;

  const cameraRaw = raw.camera;
  const camera: TreeCameraState | null =
    isRecord(cameraRaw) && isRecord(cameraRaw.position) && isRecord(cameraRaw.target)
      ? {
          position: {
            x: typeof cameraRaw.position.x === 'number' ? cameraRaw.position.x : 0,
            y: typeof cameraRaw.position.y === 'number' ? cameraRaw.position.y : 0,
            z: typeof cameraRaw.position.z === 'number' ? cameraRaw.position.z : 0,
          },
          target: {
...
function parseTreeStateV1(raw: unknown): PersistedTreeStateV2 | null {
  ...
  const rootNodeId =
    typeof raw.rootNodeId === 'string' || raw.rootNodeId === null ? raw.rootNodeId : null;
...
export function useTreeState() {
  ...
  const [rootNodeId, setRootNodeId] = useState<string | null>(initial.rootNodeId);
