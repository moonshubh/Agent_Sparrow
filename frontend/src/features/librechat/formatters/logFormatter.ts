...
export function formatLogAnalysisResult(raw: any): string | null {
  ...
  if (data && typeof data === "object" && Array.isArray((data as any).items)) {
    ...
    if (unwrappedItems.length === 1) {
      ...
      for (const item of unwrappedItems) {
        if (!merged.overall_summary && (item as any).overall_summary)
          merged.overall_summary = (item as any).overall_summary;
        if (!merged.summary && (item as any).summary)
          merged.summary = (item as any).summary;
        if (!merged.health_status && (item as any).health_status)
          merged.health_status = (item as any).health_status;
        if (
          merged.confidence_level === undefined &&
          (item as any).confidence_level !== undefined
        ) {
        ...
        const solutions =
          (item as any).proposed_solutions || (item as any).solutions;
        if (Array.isArray(solutions))
          merged.proposed_solutions.push(...solutions);
      }
  ...
  if (data && typeof data === "object") {
    const customerReady =
      (data as any).customer_ready || (data as any).customerReady;
  ...
  const concerns: any[] = Array.isArray(data.priority_concerns)
    ? data.priority_concerns
    : [];
  const issues: any[] = Array.isArray(data.identified_issues || data.issues)
    ? data.identified_issues || data.issues
    : [];
  const solutions: any[] = Array.isArray(
    data.proposed_solutions || data.solutions,
  )
    ? data.proposed_solutions || data.solutions
    : [];
  const recommendedActions: string[] = Array.isArray(
    (data as any).recommended_actions,
  )
    ? (data as any).recommended_actions.filter(
  ...
  if (solutions.length) {
    ...
    solutions.forEach((solution, index) => {
      ...
      const steps: string[] = Array.isArray(solution.steps)
        ? solution.steps
        : [];
