...
function useInlineArtifacts(content: string): string {
  ...
  return useMemo(() => {
    ...
    updated = updated.replace(artifactIdRegex, (match, alt, ref) => {
      ...
      const imageArtifact = resolveArtifactImageByRef(
        artifactsById,
        normalizedRef,
        alt || normalizedRef,
      );
...
export function ArticleEditor({ artifact }: ArticleEditorProps) {
  const { addArtifact, getArtifact, getArtifactsByMessage } =
    useArtifactStore();
  const { sessionId, updateMessageMetadata, resolvePersistedMessageId } =
    useAgent();
  ...
  const persistArtifacts = useCallback(
    async (messageId: string) => {
      ...
      const artifacts = getArtifactsByMessage(messageId).filter(
        (item) =>
          item.type === "article" ||
          item.type === "kb-article" ||
          item.type === "image",
      );
      ...
      const serialized = artifacts
        ...
        .filter(
          (item) =>
            item.type !== "image" || Boolean(item.imageUrl || item.imageData),
        );
  ...
  const handleSave = useCallback(
    async (markdown: string) => {
      ...
      try {
        ...
        console.error(
          "[ArticleEditor] Failed to persist artifact edits",
          error,
        );
        throw error;
      }
    ...
    (): Partial<Components> => ({
      ...
      h2: ({
        ...
        <h2
          {...props}
          className={cn(
            "text-xl font-semibold mt-5 mb-2 text-foreground",
            className,
          )}
        >
          {children}
      ...
      h3: ({
        ...
        <h3
          {...props}
          className={cn(
            "text-lg font-medium mt-4 mb-2 text-foreground",
            className,
          )}
        >
          {children}
      ...
      ul: ({
        ...
        <ul
          {...props}
          className={cn(
            "list-disc list-inside my-3 space-y-1 text-foreground/90",
            className,
          )}
        >
          {children}
      ...
      code: ({
        ...
        const isInlineCode =
          inline ?? !String(className ?? "").includes("language-");
        return isInlineCode ? (
          <code
            {...props}
            className={cn(
                ...
                {!hasTextContent(content) && (
                  <div className="mb-4 p-3 rounded-lg bg-amber-500/10 border border-amber-500/30">
                    <p className="text-amber-400 text-sm flex items-center gap-2 m-0">
                      ...
                      This article contains images. Click Edit to add text
                      content.
                    </p>
                  </div>
                )}
