...
export function findImageByTitle(
  ...
  const imageArtifacts = Object.values(artifactsById).filter(
    (a) => a.type === "image",
  );
...
export function resolveArtifactImageByRef(
  ...
  const exactIdMatch = Object.values(artifactsById).find(
    (artifact) =>
      artifact.type === "image" && artifact.id.toLowerCase() === refLower,
  );
  if (exactIdMatch) return exactIdMatch;

  const partialIdMatch = Object.values(artifactsById).find(
    (artifact) =>
      artifact.type === "image" && artifact.id.toLowerCase().includes(refLower),
  );
