...
import {
  ...
  type UsageStats,
} from "@/services/api/endpoints/rateLimitApi";
...
export function RateLimitsPanel() {
  ...
  const selectedBuckets = useMemo(() => {
    ...
    const imageBucketKey = buckets["internal.image"]
      ...
      : findBucketKey(
          (bucket) =>
            bucket.provider === "google" && bucket.model.includes("pro-image"),
        );

    return {
      coordinatorBucketKey,
      imageBucketKey,
      coordinator: coordinatorBucketKey
        ? buckets[coordinatorBucketKey]
        : undefined,
  ...
  const gridStats = useMemo(() => {
    ...
    return [
      {
        value: ratio(coordinator?.rpm_used, coordinator?.rpm_limit),
        label: "Gemini 3 Flash RPM",
      },
      {
        value: ratio(coordinator?.tpm_used, coordinator?.tpm_limit ?? null),
        label: "Gemini 3 Flash TPM",
      },
      {
        value: ratio(coordinator?.rpd_used, coordinator?.rpd_limit),
        label: "Gemini 3 Flash RPD",
      },
      {
        value: ratio(image?.rpm_used, image?.rpm_limit),
        label: "Gemini 3 Pro Image RPM",
      },
      {
        value: ratio(image?.tpm_used, image?.tpm_limit ?? null),
        label: "Gemini 3 Pro Image TPM",
      },
      {
        value: ratio(image?.rpd_used, image?.rpd_limit),
        label: "Gemini 3 Pro Image RPD",
      },
    ];
  }, [selectedBuckets]);
  ...
  return (
    <section className="py-4">
      <div className="mx-auto max-w-7xl px-0">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-foreground/70 max-w-2xl text-sm">
              Live usage and provider limits (Gemini, OpenAI, Tavily)
            </p>
          </div>
          <Button
            size="sm"
        ...
        {stats &&
        (!selectedBuckets.coordinatorBucketKey ||
          !selectedBuckets.imageBucketKey) ? (
          <div className="mt-2 rounded-md border border-border/50 bg-muted/20 p-3 text-xs text-muted-foreground">
            Some rate-limit buckets are not available in this deployment; the
            panel is showing best-effort values.
          </div>
        ) : null}
        ...
        <div className="grid gap-4 md:grid-cols-2">
              ...
              <ul className="text-sm text-muted-foreground space-y-1">
                <li>
                  ...
                  <span className="font-medium text-foreground">
                    {stats.total_requests_this_minute}
                  </span>
                </li>
                <li>
                  Requests today:{" "}
          ...
          <div className="rounded-lg border p-4">
            <h3 className="text-sm font-medium mb-2 flex items-center gap-2">
              <Shield className="h-4 w-4" /> Circuit Breakers
            </h3>
            {stats ? (
              <ul className="text-sm text-muted-foreground space-y-1">
                {(() => {
                  ...
                  const imageBucketKey =
                    selectedBuckets.imageBucketKey ?? "internal.image";
                  const coordinatorCircuit =
                    stats.circuits?.[coordinatorBucketKey];
        ...
        <div className="text-xs text-muted-foreground space-y-1">
          <p>
            Google Gemini free-tier limits vary; see docs:
