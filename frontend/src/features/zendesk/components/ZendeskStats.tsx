...
function StatItem({ label, value, subtitle, status = 'healthy', delay = 0 }: StatItemProps) {
  const statusColors = {
    ...
    >
      ...
      {subtitle && (
        <div className="flex items-center gap-1 mt-1 text-xs text-muted-foreground">
          {status === 'healthy' && <CheckCircle2 className="w-3 h-3" style={{ color: statusColors[status] }} />}
          {status === 'warning' && <AlertTriangle className="w-3 h-3" style={{ color: statusColors[status] }} />}
          {status === 'critical' && <AlertTriangle className="w-3 h-3" style={{ color: statusColors[status] }} />}
          <span>{subtitle}</span>
        </div>
      )}
    </motion.div>
...
function ProgressBar({ value, max, label }: { value: number; max: number; label: string }) {
  ...
  const color = percentage > 80 ? 'hsl(0 70% 50%)' : percentage > 60 ? 'hsl(40 90% 50%)' : 'hsl(135 45% 45%)'
      ...
      <div className="mx-auto max-w-7xl">
        ...
        >
          <div className={cn(
            ...
            <div className={cn("w-1.5 h-1.5 rounded-full", enabled ? "bg-green-500" : "bg-red-500")} />
          ...
          <div className={cn(
            ...
            <div className={cn("w-1.5 h-1.5 rounded-full", dryRun ? "bg-amber-500" : "bg-muted-foreground")} />
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="text-center p-2 rounded bg-amber-500/10">
                <div className="text-xl font-bold text-amber-600 dark:text-amber-400">{qPending}</div>
                <div className="text-xs text-muted-foreground">Pending</div>
              </div>
              <div className="text-center p-2 rounded bg-blue-500/10">
                <div className="text-xl font-bold text-blue-600 dark:text-blue-400">{qProcessing}</div>
                <div className="text-xs text-muted-foreground">Processing</div>
              </div>
              <div className="text-center p-2 rounded bg-purple-500/10">
                <div className="text-xl font-bold text-purple-600 dark:text-purple-400">{qRetry}</div>
                <div className="text-xs text-muted-foreground">Retry</div>
              </div>
              <div className="text-center p-2 rounded bg-red-500/10">
                <div className="text-xl font-bold text-red-600 dark:text-red-400">{qFailed}</div>
