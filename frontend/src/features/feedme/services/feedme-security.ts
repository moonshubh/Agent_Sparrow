...
const ALLOWED_FILE_TYPES = [".txt", ".pdf", ".doc", ".docx"] as const;
type AllowedFileType = (typeof ALLOWED_FILE_TYPES)[number];

const isAllowedFileType = (value: string): value is AllowedFileType =>
  ALLOWED_FILE_TYPES.includes(value as AllowedFileType);
...
export const ValidationSchemas = {
  conversationTitle: z
    ...
    .max(
      SECURITY_CONSTANTS.MAX_TITLE_LENGTH,
      `Title must be ${SECURITY_CONSTANTS.MAX_TITLE_LENGTH} characters or less`,
    )
  ...
  conversationText: z
    ...
    .max(
      SECURITY_CONSTANTS.MAX_TEXT_LENGTH,
      `Text must be ${SECURITY_CONSTANTS.MAX_TEXT_LENGTH} characters or less`,
    )
    .refine((val) => !FORBIDDEN_PATTERNS.some((pattern) => pattern.test(val)), {
  ...
  folderName: z
    ...
    .max(
      SECURITY_CONSTANTS.MAX_FOLDER_NAME_LENGTH,
      `Folder name must be ${SECURITY_CONSTANTS.MAX_FOLDER_NAME_LENGTH} characters or less`,
    )
  ...
  fileUpload: z.object({
    name: z.string().refine(
      (name) => {
        ...
        return isAllowedFileType(ext);
      },
      {
        message: `File type not allowed. Allowed types: ${Array.from(SECURITY_CONSTANTS.ALLOWED_FILE_TYPES).join(", ")}`,
      },
    ),
    size: z
      .number()
      .max(
        SECURITY_CONSTANTS.MAX_FILE_SIZE,
...
export function sanitizeFileName(fileName: string): string {
  ...
  if (sanitized.length > maxLength) {
    ...
    sanitized =
      name.substring(0, maxLength - ext.length - 10) + "_truncated" + ext;
  }
