...
export default function PlatformTagSelector({
  ...
  const selectedOption = currentPlatform
    ? PLATFORM_OPTIONS[currentPlatform as PlatformKey]
    : undefined;
  ...
  const renderPlatformIcon = (
    key: string,
    option: (typeof PLATFORM_OPTIONS)[PlatformKey],
    size = "h-4 w-4",
  ) => {
    ...
    <div className={cn("relative", className)}>
      ...
      >
        ...
        <SelectContent>
          ...
          {(
            Object.entries(PLATFORM_OPTIONS) as [
              PlatformKey,
              (typeof PLATFORM_OPTIONS)[PlatformKey],
            ][]
          ).map(([key, option]) => (
            <SelectItem key={key} value={key}>
              <div className="flex items-center gap-2">
                ...
                <span>
                  {key === "both" ? "Both Windows and macOS" : option.label}
                </span>
              </div>
            </SelectItem>
          ))}
