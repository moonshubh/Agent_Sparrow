...
function DockRoot({
  ...
  const contextValue = useMemo<DockContextValue>(
    ...
    [hoveredId, baseSize, magnification, distance, springConfig, orientation, mouseX],
  )
...
const DockCard = forwardRef<HTMLButtonElement, DockCardProps>(
  ({ id, label, onClick, className, children }, forwardedRef) => {
    const { mouseX, hoveredId, setHoveredId, baseSize, magnification, distance, springConfig, orientation } =
      useDock()
    ...
    const targetSize = useTransform(distanceFromMouse, [-distance, 0, distance], [
      baseSize,
...
const Card = forwardRef<HTMLDivElement, CardProps>(({ src, alt, children, className }, forwardedRef) => (
  <div
    ref={forwardedRef}
    className={cn('relative flex size-full items-center justify-center overflow-visible', className)}
  >
    {src ? (
      <Image
        src={src}
    ...
    {children ? (
      <span className="pointer-events-none absolute inset-0 flex items-center justify-center">{children}</span>
    ) : null}
  </div>
))
  ...
  return (
    <div
      ...
      className={cn(
        ...
        orientation === 'horizontal' ? 'h-[70%] w-[1.5px]' : 'h-[1.5px] w-[70%]',
        className,
      )}
      style={{
        margin:
