...
const FolderConversationsDialog = React.memo(function FolderConversationsDialog({ isOpen, onClose, folderId, folderName, folderColor = '#6b7280' }: Props) {
  ...
  const fetchFolderConversations = useCallback(async () => {
    ...
    try {
      ...
      const enriched: ConversationItem[] = response.conversations.map((conv) => {
        const safeMetadata = (conv.metadata ?? {}) as {
          processing_tracker?: ProcessingTracker
          processing_method?: string
        }
        const tracker = safeMetadata.processing_tracker ?? {}
        const processingMethod = (conv as { processing_method?: string }).processing_method ?? safeMetadata.processing_method
        const statusMessageSource = conv as { status_message?: string; message?: string }
        ...
        return {
          ...
          processing_status: conv.processing_status as ConversationItem['processing_status'],
          progress_percentage: progress,
          processing_stage: tracker.stage,
          status_message: tracker.message ?? statusMessageSource.status_message ?? statusMessageSource.message ?? undefined,
          ...
          folder_id: (conv as { folder_id?: number | null }).folder_id ?? null,
        }
      })
