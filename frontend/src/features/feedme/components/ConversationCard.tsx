...
export function ConversationCard({
  ...
  const currentFolder = conversation.folder_id
    ? folders[conversation.folder_id]
    : null;
  const availableFolders = Object.values(folders).filter(
    (f) => f.id !== conversation.folder_id,
  );
  const platform = conversation.metadata?.tags?.platform as
    | PlatformType
    | undefined;
          ...
          {currentFolder && (
            <div className="flex items-center gap-1 text-xs text-muted-foreground">
              <FolderIcon
                size={12}
                isOpen={false}
                color={currentFolder.color || "#0095ff"}
              />
