...
export default function FolderBits({ color = '#5227FF', size = 1, items = [], className = '' }: FolderBitsProps) {
  const maxItems = 3
  ...
  return (
    <div style={scaleStyle} className={className}>
      ...
      >
        <div
          className="relative h-[82px] w-[116px] rounded-tr-[12px] rounded-br-[12px] rounded-bl-[12px] bg-[var(--folder-back-color)]"
        >
          <span
            className="absolute bottom-[98%] left-0 h-[12px] w-[36px] rounded-tl-[6px] rounded-tr-[6px] bg-[var(--folder-back-color)]"
          />
          {papers.map((item, i) => {
            let width = 'w-[88%]'
            let height = 'h-[60%]'
