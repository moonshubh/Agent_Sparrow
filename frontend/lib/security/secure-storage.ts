...
import { STORAGE_CONFIG, SECURITY_CONFIG } from '../config/constants'
...
export class SecureStorage {
  ...
  private store(key: string, value: string): void {
    if (this.config.type === StorageType.MEMORY) {
      ...
      this.memoryStorage.set(key, { raw: value } as any)
    } else {
      const storage = this.getStorage()
      if (storage) {
        try {
  ...
  private retrieve(key: string): string | null {
    if (this.config.type === StorageType.MEMORY) {
      ...
      // Handle both old format (StorageEntry) and new format (raw string)
      if (!entry) return null
      return (entry as any).raw || JSON.stringify(entry)
    }
  ...
  private sanitizeString(str: string): string {
    // Use DOMPurify if available, otherwise use enhanced regex sanitization
    if (typeof window !== 'undefined' && (window as any).DOMPurify) {
      return (window as any).DOMPurify.sanitize(str, { ALLOWED_TAGS: [] })
    }
