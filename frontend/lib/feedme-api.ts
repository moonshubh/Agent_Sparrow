...
export class FeedMeApiClient {
  ...
  async getConversationExamples(
    ...
    pageSize: number = 20,
    isActive?: boolean
  ): Promise<{ examples: any[]; total_examples: number; page: number; page_size: number; total_pages: number }> {
    console.warn('[FeedMe API] getConversationExamples is deprecated (unified text flow). Returning empty.')
    return { examples: [], total_examples: 0, page, page_size: pageSize, total_pages: 1 } as any
  }
  ...
  /**
   * Get Gemini usage counters (FeedMe in-process tracker)
   */
  async getGeminiUsage(): Promise<{
    ...
    const response = await fetchWithRetry(`${FEEDME_API_BASE}/gemini-usage`)
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}))
  ...
  /**
   * Get Embedding usage counters (TPM/RPM/RPD for embeddings)
   */
  async getEmbeddingUsage(): Promise<{
    ...
    const response = await fetchWithRetry(`${FEEDME_API_BASE}/embedding-usage`)
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}))
