...
export function ConversationEditor({ conversationId, isOpen, onClose }: ConversationEditorProps) {
  ...
  const loadConversation = useCallback(async () => {
    ...
    try {
      ...
      if (!conversation) {
        // Conversation doesn't exist, try fetching directly from API
        console.warn(`Conversation ${conversationId} not found in store, attempting direct fetch`)
        
        try {
          // Try direct API fetch as fallback
          const directConversation = await feedMeApi.getConversation(conversationId)
          
          if (directConversation) {
            // Update store with the fetched conversation
            await conversationActions.updateConversation(conversationId, directConversation)
          ...
          console.error('Failed to fetch conversation directly:', error)
        }
        
        // If still not found, show error and close
        uiActions.showToast({
          type: 'warning',
          title: 'Conversation Not Found',
          message: 'This conversation may have been deleted or is still processing.'
        })
        
        // Don't remove from store - let the list refresh handle it
