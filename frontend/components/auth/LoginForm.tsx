...
export const LoginForm: React.FC = () => {
  ...
  const handleOAuthLogin = async (provider: 'google' | 'github') => {
    ...
    try {
      ...
      const errorMessage = getErrorMessage(error, provider.charAt(0).toUpperCase() + provider.slice(1))
    ...
    <div className="space-y-4">
      ...
      {isOAuthEnabled && !oauthConfig.google.enabled && !oauthConfig.github.enabled && (
        <WarningMessage title="No Providers Available">
          No OAuth providers are configured. Please configure at least one OAuth provider.
        </WarningMessage>
      )}
