...
async function testCSRFProtection() {
  ...
  try {
    ...
    if (response.statusCode === 403) {
      console.log(
        "  ✅ CSRF protection active - POST request blocked without token",
      );
      return true;
      ...
      console.log(
        `  ❌ CSRF protection failed - Expected 403, got ${response.statusCode}`,
      );
      return false;
    }
  } catch (error) {
...
async function testAuthRoutes() {
  ...
  const testCases = [
    ...
    {
      path: "/auth/callback",
      description: "Auth callback (should be accessible)",
    },
    {
      path: "/api/health",
    ...
    {
      path: "/protected",
      description: "Protected route (should redirect to login)",
    },
  ];
    ...
    try {
      ...
      if (testCase.path === "/protected") {
        if (statusCode === 302 || statusCode === 307) {
          console.log(
            `  ✅ ${testCase.description} - Redirected (${statusCode})`,
          );
          passed++;
        } else if (statusCode === 404) {
          console.log(
            `  ⚠️  ${testCase.description} - Route not found (expected)`,
          );
          passed++;
        } else {
          console.log(
            `  ❌ ${testCase.description} - Expected redirect, got ${statusCode}`,
          );
        }
      } else {
        if (statusCode >= 200 && statusCode < 400) {
          console.log(
            `  ✅ ${testCase.description} - Accessible (${statusCode})`,
          );
          passed++;
        } else {
          console.log(
            `  ❌ ${testCase.description} - Unexpected status ${statusCode}`,
          );
        }
      }
    } catch (error) {
...
async function testRateLimiting() {
  ...
  console.log(
    "  ⚠️  Rate limiting test disabled by default to avoid overwhelming local dev",
  );
...
async function runSecurityTests() {
  ...
  console.log("\\n" + "=".repeat(50));
  console.log(
    `🎯 Security Test Results: ${passed}/${total} test suites passed`,
  );
...
if (process.argv.includes("--enable-rate-limiting")) {
  ...
  console.log(
    "⚠️  Rate limiting test enabled - this will make multiple requests",
  );
}
