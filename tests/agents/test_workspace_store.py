...
def test_search_root_empty_namespace_prefix_returns_matches() -> None:
    async def _run() -> None:
...
def test_search_root_respects_offset_pagination() -> None:
    async def _run() -> None:
        store = SparrowWorkspaceStore(
...
def test_prune_user_sessions_deletes_oldest() -> None:
    async def _run() -> None:
...
def test_prune_user_sessions_paginates_over_large_indices() -> None:
    async def _run() -> None:
        store = SparrowWorkspaceStore(
            session_id="sess-current",
            user_id="user-1",
