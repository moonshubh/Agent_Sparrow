-- Switch status CHECK constraint to NOT VALID then VALIDATE for safer rollout
DO $$
BEGIN
  IF EXISTS (
  ...
  CHECK (status IN ('pending','retry','processing','processed','failed')) NOT VALID;

  ALTER TABLE public.zendesk_pending_tickets
  VALIDATE CONSTRAINT zendesk_pending_tickets_status_check;
END $$;
