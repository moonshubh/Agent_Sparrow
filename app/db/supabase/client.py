...
class SupabaseClient:
    ...
    def table(self, name: str):
        ...
        if self.mock_mode:
            logger.debug(
                "Supabase mock_mode active - returning mock table for %s", name
            )
    ...
    def rpc(self, fn_name: str, params: Optional[Dict[str, Any]] = None):
        ...
        if self.mock_mode:
            logger.debug(
                "Supabase mock_mode active - returning mock rpc for %s", fn_name
            )
            ...
            for attempt in range(2):
                try:
                    ...
                    if attempt == 0 and (
                        ...
                        logger.warning(
                            "Supabase exec transient error (%s). Retrying once...", e
                        )
    ...
    async def insert_sparrow_correction(
        ...
        *,
        ...
        try:
            response = await self._exec(
                lambda: self.client.table("sparrow_corrections")
                .insert(payload)
                .execute()
            )
    ...
    async def get_sparrow_feedback(self, feedback_id: int) -> Optional[Dict[str, Any]]:
        try:
            response = await self._exec(
                lambda: self.client.table("sparrow_feedback")
                .select("*")
                .eq("id", feedback_id)
                .limit(1)
                .execute()
            ...
            if getattr(exc, "code", None) in {"PGRST116", "PGRST103"}:
                return None
            logger.error(
                "Supabase error fetching sparrow_feedback id=%s: %s", feedback_id, exc
            )
            return None
        except Exception as exc:
    ...
    async def get_sparrow_correction(
        ...
        try:
            ...
            if getattr(exc, "code", None) in {"PGRST116", "PGRST103"}:
                return None
            logger.error(
                "Supabase error fetching sparrow_corrections id=%s: %s",
                correction_id,
                exc,
            )
            return None
            ...
            logger.error(
                "Failed to fetch sparrow_corrections id=%s: %s", correction_id, exc
            )
            return None
    ...
    async def update_sparrow_feedback_status(
        ...
        *,
        ...
        try:
            ...
            logger.error(
                "Failed to update sparrow_feedback id=%s: %s", feedback_id, exc
            )
            return None
    ...
    async def update_sparrow_correction_status(
        ...
        *,
        ...
        try:
            ...
            logger.error(
                "Failed to update sparrow_corrections id=%s: %s", correction_id, exc
            )
            return None
    ...
    async def insert_folder(
        ...
        try:
            ...
            response = await self._exec(
                lambda: self.client.table("feedme_folders").insert(data).execute()
            )
    ...
    async def update_folder(
        self, folder_id: int, update_data: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if response.data and len(response.data) > 0:
                logger.info(
                    f"Updated folder ID: {folder_id} with fields: {list(filtered_data.keys())}"
                )
                return response.data[0]
            else:
    ...
    async def list_folders(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = await self._exec(
                lambda: self.client.table("feedme_folder_stats")
                .select("*")
                .order("folder_path")
                .execute()
            )
    ...
    async def insert_conversation(
        ...
        try:
            ...
            response = await self._exec(
                lambda: self.client.table("feedme_conversations").insert(data).execute()
            )

            if response.data:
                logger.info(
                    f"Created conversation: {title} (ID: {response.data[0]['id']})"
                )
                return response.data[0]
            else:
    ...
    async def list_conversations(
        ...
        try:
            ...
            logger.info(
                f"Listed {len(response.data)} conversations (total: {total_count})"
            )
    ...
    async def update_conversation(
        self, conversation_id: int, updates: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """
    ...
    async def delete_conversation(self, conversation_id: int) -> bool:
        ...
        try:
            ...
            try:
                chunks_response = await self._exec(
                    lambda: self.client.table("feedme_text_chunks")
                    .delete()
                ...
                logger.debug(
                    f"No text chunks to delete for conversation {conversation_id}: {e}"
                )
                chunks_count = 0
            ...
            try:
                ...
                examples_count = (
                    len(examples_response.data) if examples_response.data else 0
                )
            except Exception as e:
                logger.debug(
                    f"No examples to delete for conversation {conversation_id}: {e}"
                )
                examples_count = 0

            # Then delete the conversation
            conversation_response = await self._exec(
                lambda: self.client.table("feedme_conversations")
                .delete()
                .eq("id", conversation_id)
                .execute()
    ...
    async def update_conversation_folder(
        self, conversation_id: int, folder_id: Optional[int]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.data:
                logger.info(
                    f"Moved conversation {conversation_id} to folder {folder_id}"
                )
                return response.data[0]
            else:
    ...
    async def bulk_assign_conversations_to_folder(
        self, conversation_ids: List[int], folder_id: Optional[int]
    ) -> int:
        ...
        try:
            ...
            logger.info(
                f"Assigned {count} conversations to folder {effective_folder_id} (input: {folder_id})"
            )
            return count
    ...
    async def update_example(
        self, example_id: int, update_data: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        """
    ...
    async def get_example_with_conversation(
        self, example_id: int
    ) -> Optional[Dict[str, Any]]:
        """
    ...
    async def update_conversation_example_count(self, conversation_id: int) -> bool:
        ...
        try:
            ...
            if response.data:
                logger.info(
                    f"Updated conversation {conversation_id} example count to {example_count}"
                )
                return True
            else:
                logger.warning(
                    f"Conversation {conversation_id} not found for example count update"
                )
                return False
    ...
    async def approve_examples(
        self, example_ids: List[int], approved_by: str
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def insert_examples(
        self, examples: List[Dict[str, Any]], mark_approved: bool = True
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            response = await self._exec(
                lambda: self.client.table("feedme_examples").insert(examples).execute()
            )
    ...
    async def search_examples(
        ...
        try:
            ...
            if folder_ids:
                ...
                filters.append(
                    f"conversation_id IN (SELECT id FROM feedme_conversations WHERE folder_id = ANY(ARRAY[{','.join(map(str, folder_ids))}]))"
                )
            ...
            response = await self._exec(
                lambda: self.rpc("search_feedme_examples", params).execute()
            )
    ...
    async def search_kb_articles(
        ...
        try:
            if self.mock_mode:
                logger.info(
                    "search_kb_articles called in mock mode; returning empty list"
                )
                return []
            ...
            try:
                ...
                similarity_threshold = (
                    0.0 if similarity_threshold < 0 else similarity_threshold
                )
                similarity_threshold = (
                    1.0 if similarity_threshold > 1 else similarity_threshold
                )
            except Exception:
            ...
            response = await self._exec(
                lambda: self.rpc("search_mailbird_knowledge", params).execute()
            )
            return response.data or []
        except APIError as e:
            try:
                dim = (
                    len(query_embedding) if isinstance(query_embedding, list) else None
                )
            except Exception:
                dim = None
    ...
    async def search_web_snapshots(
        ...
        try:
            if self.mock_mode:
                logger.info(
                    "search_web_snapshots called in mock mode; returning empty list"
                )
                return []
    ...
    async def get_conversation_analytics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await self._exec(
                lambda: self.rpc("get_conversation_analytics").execute()
            )
    ...
    async def get_conversation_processing_stats(
        self, conversation_id: int
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            high_quality = len(
                [e for e in examples if e.get("confidence_score", 0) >= 0.8]
            )
            medium_quality = len(
                [e for e in examples if 0.5 <= e.get("confidence_score", 0) < 0.8]
            )
            low_quality = len(
                [e for e in examples if e.get("confidence_score", 0) < 0.5]
            )

            avg_confidence = sum(
                [e.get("confidence_score", 0) for e in examples]
            ) / max(total_examples, 1)
            avg_usefulness = sum(
                [e.get("usefulness_score", 0) for e in examples]
            ) / max(total_examples, 1)
    ...
    async def get_approval_workflow_stats(self) -> Dict[str, Any]:
        ...
        try:
            # Use optimized RPC function for single-query aggregation
            response = await self._exec(
                lambda: self.rpc("get_approval_workflow_stats").execute()
            )
    ...
    async def bulk_reprocess_conversations(
        ...
        try:
            ...
            for conversation_id in conversation_ids:
                try:
                    ...
                    update_result = await self._exec(
                        lambda cid=cid: self.client.table("feedme_conversations")
                        .update(
                            {
                                "processing_status": "pending",
                                "error_message": None,
                    ...
                    if update_result.data:
                        ...
                        logger.info(
                            f"Queued conversation {conversation_id} for reprocessing"
                        )
                    else:
                        failed.append(
                            {"id": conversation_id, "error": "Conversation not found"}
                        )

                except Exception as e:
                    failed.append({"id": conversation_id, "error": str(e)})
                    logger.error(
                        f"Failed to reprocess conversation {conversation_id}: {e}"
                    )
    ...
    async def delete_text_chunks_for_conversation(self, conversation_id: int) -> int:
        ...
        try:
            ...
            logger.error(
                f"Failed to delete text chunks for conversation {conversation_id}: {e}"
            )
            return 0
    ...
    async def insert_text_chunk(
        ...
        try:
            ...
            response = await self._exec(
                lambda: self.client.table("feedme_text_chunks").insert(data).execute()
            )
            if response.data:
                return response.data[0]
    ...
    async def update_text_chunk_embedding(
    ...
    async def search_text_chunks(
        ...
        try:
            ...
            result = await self._exec(
                lambda: self.rpc("search_feedme_text_chunks", params).execute()
            )
        ...
        try:
            response = await self._exec(
                lambda: (
                    self.client.table("feedme_conversations")
                    .select(
                        "id,title,metadata,extracted_text,approval_status,processing_status,folder_id,created_at"
                    )
                    .in_("id", conversation_ids)
                    .execute()
                )
    ...
    async def get_feedme_summary(self) -> Dict[str, Any]:
        ...
        try:
            # Use optimized RPC function for single-query aggregation
            response = await self._exec(
                lambda: self.rpc("get_feedme_summary").execute()
            )
            ...
            if response.data:
                ...
                result = {
                    "overview": {
                        ...
                        "examples_from_conversations": conv_data.get(
                    ...
                    "system_health": {
                        ...
                        "pending_approval": approval_stats.get("pending", 0)
                        + approval_stats.get("processed", 0),
    ...
    async def get_folders_with_stats(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            folders_response = await self._exec(
                lambda: (
                    self.client.table("feedme_folders")
                    .select(
                        "id,name,color,description,created_by,created_at,updated_at,feedme_conversations(count)"
                    )
                    .order("name")
                    .execute()
                )
            ...
            no_folder_response = await self._exec(
                lambda: (
                    self.client.table("feedme_conversations")
                    .select("id", count=COUNT_EXACT)
                    .is_("folder_id", "null")
    ...
    async def move_conversations_to_folder(
        self, conversation_ids: List[int], target_folder_id: Optional[int]
    ) -> Dict[str, Any]:
        """
    ...
    async def bulk_update_examples(
        self, example_ids: List[int], update_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def get_examples_by_conversation(
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(examples)} examples for conversation {conversation_id}"
            )
            return examples
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await self._exec(
                lambda: self.client.table("feedme_folders")
                .select("*", count=COUNT_EXACT)
                .limit(0)
                .execute()
            )

            # Test vector extension
            vector_test = await self._exec(
                lambda: self.rpc("test_vector_extension").execute()
            )
