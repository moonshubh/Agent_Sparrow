...
def get_db_connection():
    ...
    if not database_url:
        ...
        if not all([host, user, password, dbname]):
            logger.error("Database connection details are missing; cannot open connection.")
            return None
        connection_params = {
            "host": host,
            "user": user,
    ...
    try:
        conn = psycopg2.connect(connection_params) if isinstance(connection_params, str) else psycopg2.connect(**connection_params)
