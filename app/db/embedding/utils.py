...
def _ensure_pgvector_stub() -> None:
    """Provide a lightweight pgvector stub when the extension is unavailable."""
...
from app.db.embedding_config import (
    MODEL_NAME as EMB_MODEL_NAME_SOT,
    EXPECTED_DIM as EXPECTED_EMBEDDING_DIM,
    assert_dim,
)
...
class ResilientEmbeddings(Embeddings):
    ...
    def _should_use_fallback(self, exc: Exception, context: str) -> bool:
        if not self._allow_fallback:
            raise exc
        logger.warning(
            "Primary embedding %s failed: %s -- using fallback", context, exc
        )
        return True
    ...
    async def aembed_documents(
        self, texts: List[str]
    ) -> List[List[float]]:  # pragma: no cover - async path
        if self._prefer_fallback:
...
def get_db_connection():
    """
    Establish a PostgreSQL connection using DATABASE_URL or discrete DB_* env vars.
    Returns None when credentials are missing or a connection cannot be established.
    """
    database_url = os.getenv("DATABASE_URL") or settings.supabase_db_conn
    if not database_url:
        ...
        if not all([host, user, password, dbname]):
            logger.error(
                "Database connection details are missing; cannot open connection."
            )
            return None
        connection_params = {
            "host": host,
    ...
    try:
        conn = (
            ...
            else psycopg2.connect(**connection_params)
        )
        register_vector(conn)
        logger.info("Established PostgreSQL connection for embedding operations.")
        return conn
    except Exception as exc:
...
def generate_embeddings_for_pending_content(batch_size: int = 10) -> int:
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2_extras.NamedTupleCursor) as cur:
            ...
            cur.execute(
                "SELECT id, COALESCE(markdown, content) as text_content FROM mailbird_knowledge WHERE embedding IS NULL LIMIT %s;",
                (batch_size,),
            )
            ...
            logger.info(
                f"Found {len(records_to_process)} records to process for embeddings."
            )
            ...
            for record in records_to_process:
                try:
                    ...
                    if (
                        not record.text_content
                        or not isinstance(record.text_content, str)
                        or not record.text_content.strip()
                    ):
                        logger.warning(
                            f"Skipping record ID {record.id} due to missing, invalid, or empty text_content."
                        )
                    ...
                    if len(text_to_embed) > MAX_CHARS_FOR_EMBEDDING:
                        ...
                        logger.warning(
                            f"Record ID {record.id}: Content truncated for embedding. Original: {original_length} chars, Truncated: {len(text_to_embed)} chars."
                        )

                    embedding_vector = emb_model.embed_query(text_to_embed)
                    if not _embedding_has_expected_dim(
                        embedding_vector, "generate_embeddings_for_pending_content"
                    ):
                        conn.rollback()
                        logger.error(
                    ...
                    update_sql = (
                        "UPDATE mailbird_knowledge SET embedding = %s WHERE id = %s;"
                    )
                    ...
                    logger.info(
                        f"Successfully generated and stored embedding for record ID: {record.id}"
                    )
                    updated_count += 1
...
def find_similar_documents(query: str, top_k: int = 5) -> List[SearchResult]:
    ...
    logger.info(
        f"Performing similarity search for query: '{query[:50]}...' with top_k={top_k}"
    )
    conn = None
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2_extras.NamedTupleCursor) as cur:
            ...
            for res in results:
                logger.debug(
                    f"  ID: {res.id}, URL: {res.url}, Score: {res.similarity_score:.4f}"
                )
...
def find_similar_feedme_examples(
    ...
    """
    logger.info(
        f"Performing FeedMe similarity search for query: '{query[:50]}...' with top_k={top_k}"
    )
    conn = None
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2_extras.NamedTupleCursor) as cur:
            ...
            for res in results:
                logger.debug(
                    f"  ID: {res.id}, Conversation: {res.conversation_title}, Score: {res.similarity_score:.4f}"
                )
...
def find_combined_similar_content(
    ...
    """
    logger.info(
        f"Performing combined similarity search for query: '{query[:50]}...' with top_k_total={top_k_total}"
    )
    ...
    try:
        ...
        kb_top_k = max(
            1, int(top_k_total * kb_weight * 1.5)
        )  # Get a few extra to ensure variety
        ...
        if settings.feedme_enabled:
            feedme_top_k = max(
                1, int(top_k_total * feedme_weight * 1.5)
            )  # Get a few extra
            feedme_results = find_similar_feedme_examples(
                query, top_k=feedme_top_k, min_similarity=min_feedme_similarity
            )
            ...
            for feedme_result in feedme_results:
                combined_result = CombinedSearchResult(
                    ...
                    similarity_score=feedme_result.similarity_score
                    * feedme_weight,  # Apply weight
                    additional_data={
        ...
        logger.info(
            f"Combined search found {len(combined_results)} total results "
        ...
        for i, result in enumerate(combined_results[:5]):  # Log top 5
            logger.debug(
                f"  {i + 1}. {result.source_type}: {result.title[:50]}... (Score: {result.similarity_score:.4f})"
            )
...
def generate_feedme_embeddings(
    ...
    try:
        ...
        if not force_regenerate:
            conditions.append(
                "(question_embedding IS NULL OR answer_embedding IS NULL OR combined_embedding IS NULL)"
            )
        ...
        with conn.cursor(cursor_factory=psycopg2_extras.NamedTupleCursor) as cur:
            ...
            logger.info(
                f"Found {len(records_to_process)} FeedMe examples to process for embeddings."
            )
            ...
            for record in records_to_process:
                try:
                    ...
                    if not _embedding_has_expected_dim(
                        combined_embedding, "generate_feedme_embeddings:combined"
                    ):
                    ...
                    logger.info(
                        f"Successfully generated embeddings for FeedMe example ID: {record.id}"
                    )
                    updated_count += 1
        ...
        logger.info("--- FeedMe Embedding Generation Summary ---")
...
if __name__ == "__main__":
    ...
    while True:
        logger.info(
            f"Processing next batch of up to {batch_size} records for embeddings..."
        )
        try:
            processed_in_batch = generate_embeddings_for_pending_content(
            ...
            if processed_in_batch < batch_size:
                logger.info(
                    f"Processed {processed_in_batch} records. Fewer than batch size ({batch_size}), assuming all pending records are done or no more found."
                )
                break
            if processed_in_batch == 0:
                logger.info(
                    "No records processed in this batch. All pending records should be done."
                )
                break
            logger.info(
                f"Successfully processed {processed_in_batch} records in this batch. Fetching next batch..."
            )
        except Exception as e:
            logger.error(
                f"An error occurred during a batch of embedding generation: {e}",
                exc_info=True,
            )
            logger.error("Stopping further batch processing due to error.")
            break
    logger.info(
        f"Embedding generation process finished. Total records for which embeddings were attempted/generated: {total_records_embedded}"
    )
    ...
    if settings.feedme_enabled:
        ...
        logger.info(
            f"FeedMe embedding generation finished. Total examples processed: {feedme_embedded}"
        )
    ...
    if similar_docs:
        logger.info(
            f"Top {len(similar_docs)} KB documents similar to '{sample_query}':"
        )
    ...
    if settings.feedme_enabled:
        feedme_examples = find_similar_feedme_examples(
            sample_query, top_k=3, min_similarity=0.5
        )
        if feedme_examples:
            logger.info(
                f"Top {len(feedme_examples)} FeedMe examples similar to '{sample_query}':"
            )
            for example in feedme_examples:
                logger.info(
                    f"  - {example.conversation_title}: {example.question_text[:50]}... (Score: {example.similarity_score:.4f})"
                )
        else:
        ...
        if combined_results:
            logger.info(
                f"Top {len(combined_results)} combined results for '{sample_query}':"
            )
            for result in combined_results:
                logger.info(
                    f"  - [{result.source_type}] {result.title[:50]}... (Score: {result.similarity_score:.4f})"
                )
        else:
