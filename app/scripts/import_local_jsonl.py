...
def get_db_connection():
    ...
    if not DATABASE_URL:
        logger.error(
            "DATABASE_URL not found in environment variables. Please set it in your .env file."
        )
...
def parse_timestamp(date_string: str) -> datetime:
    ...
    try:
        ...
        logger.warning(
            f"Could not parse date string '{date_string}': {e}. Using current timestamp."
        )
        return datetime.now(timezone.utc)
...
def process_jsonl_file(file_path: Path, conn) -> tuple[int, int]:
    ...
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            for line_number, line in enumerate(f, 1):
                ...
                try:
                    ...
                    logger.warning(
                        f"Skipping malformed JSON line {line_number} in {file_path}: {e}"
                    )
                    continue
                ...
                if not url:
                    logger.warning(
                        f"Skipping line {line_number} in {file_path} due to missing URL."
                    )
                    continue
                ...
                if not content:
                    logger.warning(
                        f"Skipping URL {url} from {file_path} due to missing content ('articleBody' or 'text')."
                    )
                    continue
                ...
                metadata = {
                    k: v for k, v in data.items() if k not in METADATA_EXCLUDE_FIELDS
                }
                ...
                if len(records_to_insert) >= BATCH_SIZE:
                    inserted_updated_count += _execute_batch_insert(
                        conn, records_to_insert
                    )
                    records_to_insert = []
        ...
        return (
            processed_lines,
            0,
        )  # Return lines processed so far, but 0 successful inserts for this file

    logger.info(
        f"Finished processing {file_path}. Processed {processed_lines} lines. Inserted/Updated {inserted_updated_count} records."
    )
...
def _execute_batch_insert(conn, records: list) -> int:
    ...
    try:
        with conn.cursor() as cur:
            ...
            return len(
                records
            )  # Assume all records in batch were processed (inserted or updated)
    except psycopg2.Error as e:
...
def main():
    ...
    try:
        ...
        logger.info(
            f"Total records inserted/updated in the database: {total_records_imported_updated}"
        )
