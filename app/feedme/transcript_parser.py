...
class ExtractedQA(BaseModel):
    ...
    quality: ExtractionQuality = Field(
        default=ExtractionQuality.MEDIUM, description="Quality assessment"
    )
    tags: List[str] = Field(
        default_factory=list, description="Automatically detected tags"
    )
    issue_type: Optional[str] = Field(None, description="Detected issue category")
    resolution_type: Optional[str] = Field(
        None, description="Type of resolution provided"
    )
    extraction_method: str = Field(
        ..., description="Method used for extraction ('ai' or 'pattern')"
    )
    start_position: int = Field(
        default=0, description="Character position in original transcript"
    )
    end_position: int = Field(
        default=0, description="End character position in original transcript"
    )
...
class TranscriptParsingResult(BaseModel):
    ...
    detected_format: TranscriptFormat = Field(
        ..., description="Detected transcript format"
    )
    parsing_strategy: ParsingStrategy = Field(
        ..., description="Strategy used for parsing"
    )
    extracted_examples: List[ExtractedQA] = Field(
        ..., description="Extracted Q&A examples"
    )
    total_examples: int = Field(..., description="Total number of examples extracted")
    high_quality_count: int = Field(
        default=0, description="Number of high-quality examples"
    )
    processing_time_seconds: float = Field(
        default=0.0, description="Time taken to process"
    )
    errors: List[str] = Field(
        default_factory=list, description="Parsing errors encountered"
    )
...
class TranscriptParser:
    ...
    def __init__(self, preview_char_limit: int | None = None):
        self.ai_model = None
        self.preview_char_limit = preview_char_limit or int(
            os.getenv("FEEDME_TRANSCRIPT_PREVIEW_CHARS", "3000")
        )
    ...
    def parse_transcript(
        ...
        logger.info(
            f"Starting transcript parsing (strategy: {parsing_strategy.value}, max_examples: {max_examples})"
        )
        ...
        try:
            ...
            cleaned_transcript = self._preprocess_transcript(
                transcript, transcript_format
            )

            # Extract Q&A examples based on strategy
            if parsing_strategy == ParsingStrategy.SIMPLE_PATTERN:
                examples = self._extract_with_patterns(
                    cleaned_transcript, transcript_format
                )
            ...
            high_quality_count = len(
                [ex for ex in examples if ex.quality == ExtractionQuality.HIGH]
            )
            ...
            result = TranscriptParsingResult(
                ...
                metadata={
                    ...
                    "avg_confidence": (
                        sum(ex.confidence_score for ex in examples) / len(examples)
                        if examples
                        else 0.0
                    ),
                },
            ...
            logger.info(
                f"Parsing completed: {len(examples)} examples extracted in {processing_time:.2f}s"
            )
            return result
    ...
    def _detect_format(self, transcript: str) -> TranscriptFormat:
        ...
        email_patterns = [r"From:.*@", r"To:.*@", r"Subject:", r"Reply to this email"]

        if any(
            re.search(pattern, transcript, re.IGNORECASE) for pattern in email_patterns
        ):
            return TranscriptFormat.EMAIL_THREAD

        # Check for support ticket patterns
        ticket_patterns = [r"Ticket #\d+", r"Case #\d+", r"Priority:", r"Status:"]

        if any(
            re.search(pattern, transcript, re.IGNORECASE) for pattern in ticket_patterns
        ):
    ...
    def _preprocess_transcript(
        self, transcript: str, format_type: TranscriptFormat
    ) -> str:
        ...
        if format_type == TranscriptFormat.CHAT_LOG:
            ...
            cleaned = re.sub(
                r"^(From|To|Subject|Date):.*$", "", cleaned, flags=re.MULTILINE
            )
            ...
            cleaned = re.sub(
                r"^(Ticket|Case|Priority|Status).*?:", "", cleaned, flags=re.MULTILINE
            )
                    ...
                    if match:
                        ...
                        qa = ExtractedQA(
                            ...
                            context_after=self._get_context(
                                lines, i + 1, min(len(lines), i + 3)
                            ),
                            confidence_score=0.7,  # Pattern matching confidence
                            quality=self._assess_quality(current_question, answer),
                            tags=self._extract_tags(current_question, answer),  # type: ignore[attr-defined]
                            issue_type=self._classify_issue(current_question),  # type: ignore[attr-defined]
                            resolution_type=self._classify_resolution(answer),  # type: ignore[attr-defined]
                    ...
                    for item in qa_data:
                        qa = ExtractedQA(
                            ...
                            confidence_score=min(
                                1.0, max(0.0, item.get("confidence", 0.5))
                            ),
                            tags=item.get("tags", []),
    ...
    def _extract_hybrid(
        ...
        if format_type == TranscriptFormat.HTML_ZENDESK:
            try:
                ...
                if html_examples:
                    logger.info(
                        f"HtmlZendeskParser extracted {len(html_examples)} examples"
                    )
                    return html_examples
                else:
                    logger.warning(
                        "HtmlZendeskParser found no examples, falling back to hybrid extraction"
                    )
            except Exception as e:
                logger.error(
                    f"HtmlZendeskParser failed: {e}, falling back to hybrid extraction"
                )
    ...
    def _assess_quality(self, question: str, answer: str) -> ExtractionQuality:
        ...
        has_question_indicator = any(
            indicator in question.lower() for indicator in question_indicators
        )
        ...
        has_solution_indicator = any(
            indicator in answer.lower() for indicator in solution_indicators
        )
...
class HtmlZendeskParser:
    ...
    def parse(self) -> List[ExtractedQA]:
        ...
        try:
            ...
            logger.info(
                f"HtmlZendeskParser extracted {len(pairs)} Q&A pairs from {len(messages)} messages"
            )
            return pairs
        except Exception as e:
        ...
        try:
            ...
            for comment_div in self.soup.select("div.zd-comment"):
                try:
                    ...
                    # Extract timestamp
                    timestamp = (
                        self._parse_timestamp(time_tag.get_text()) if time_tag else None
                    )
            ...
            if msg["role"] == "Customer":
                ...
                try:
                    qa = ExtractedQA(
                        ...
                        quality=self._assess_quality(
                            current_question["text"], msg["text"]
                        ),
                        context_before="",
                        context_after="",
                        tags=self._extract_tags(current_question["text"], msg["text"]),  # type: ignore[attr-defined]
                        issue_type=self._classify_issue(current_question["text"]),  # type: ignore[attr-defined]
                        resolution_type=self._classify_resolution(msg["text"]),  # type: ignore[attr-defined]
    ...
    def _classify_issue(self, question: str) -> Optional[str]:
        ...
        if any(word in question_lower for word in ["setup", "configure", "install"]):
            ...
            word in question_lower for word in ["problem", "issue", "error", "bug"]
        ):
    ...
    def _classify_resolution(self, answer: str) -> Optional[str]:
        ...
        if any(
            ...
            for word in ["step 1", "first", "then", "next", "follow"]
        ):
            return "step-by-step-guide"
        elif any(
            word in answer_lower for word in ["settings", "configure", "change", "set"]
        ):
            return "configuration-change"
        elif any(
            word in answer_lower for word in ["workaround", "temporary", "alternative"]
        ):
            return "workaround"
