...
class ApprovalStateMachine:
    ...
    def transition(
        self, current_state: ApprovalState, action: ApprovalAction
    ) -> ApprovalState:
        """
        ...
        logger.info(f"State transition: {current_state} --({action})--> {new_state}")
    ...
    def get_workflow_path(
        self, start_state: ApprovalState, end_state: ApprovalState
    ) -> Optional[List[ApprovalAction]]:
        """
        ...
        queue: deque[tuple[ApprovalState, list[ApprovalAction]]] = deque(
            [(start_state, [])]
        )
        visited = {start_state}
    ...
    def get_state_statistics(self) -> Dict[str, Any]:
        ...
        final_states = sum(
            1 for state in self._valid_states if self.is_final_state(state)
        )
