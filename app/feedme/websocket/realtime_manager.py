...
from typing import Awaitable, Dict, List, Optional, Any, TypeVar, cast
...
class FeedMeRealtimeManager:
    ...
    def __init__(self, redis_url: Optional[str] = None):
        ...
        if redis_url:
            try:
                ...
                logger.warning(
                    f"Failed to connect to Redis: {e}. Using in-memory storage only."
                )
    ...
    async def _await_redis(self, result: Awaitable["T"] | "T") -> "T":
        """Handle redis methods that may be sync or async depending on client."""
        ...
        try:
            ...
            if connection_info:
                self._log_connection_event(
                    "disconnect", connection_info.user_id, room_id
                )
    ...
    async def broadcast_to_room(
        ...
        for websocket, connection_info in room.connections.items():
            ...
            if (
                required_permission
                and required_permission not in connection_info.permissions
            ):
                continue
    ...
    async def broadcast_processing_update(self, update: ProcessingUpdate):
        ...
        """
        # Create message
        message = {"type": MessageType.PROCESSING_UPDATE, **update.model_dump()}
        ...
        await self.broadcast_to_room(
            "processing_updates", message, required_permission="processing:read"
        )
    ...
    async def broadcast_approval_update(self, update: ApprovalUpdate):
        ...
        """
        # Create message
        message = {"type": MessageType.APPROVAL_UPDATE, **update.model_dump()}

        # Broadcast to approval workflow room
        await self.broadcast_to_room(
            "approval_updates", message, required_permission="approval:read"
        )
    ...
    async def queue_message_for_user(
        self, user_id: str, message: Dict[str, Any], ttl_seconds: int = 3600
    ):
        """
        ...
        queued_message = QueuedMessage(
            user_id=user_id, message=message, ttl_seconds=ttl_seconds
        )
        ...
        if self.redis_enabled and self.redis_client:
            try:
                ...
                message_data = {
                    "message": message,
                    "created_at": queued_message.queued_at.isoformat(),
                    "ttl_seconds": ttl_seconds,
                }
                await self._await_redis(
                    self.redis_client.lpush(redis_key, json.dumps(message_data))
                )
                await self._await_redis(
                ...
                logger.error(
                    f"Failed to queue message in Redis: {e}. Using in-memory queue."
                )
    ...
    def get_metrics(self) -> WebSocketMetrics:
        ...
        messages_per_minute = (
            len(recent_events) / self.metrics_window.total_seconds() * 60
        )
    ...
    async def _validate_connection(
        self, user_id: str, room_id: str, permissions: List[str]
    ):
        """Validate connection parameters"""
    ...
    async def _send_connection_status(
        self, websocket: WebSocket, connection_info: ConnectionInfo, status: str
    ):
    ...
    async def _deliver_queued_messages(self, user_id: str, websocket: WebSocket):
        ...
        if self.redis_enabled and self.redis_client:
            try:
                redis_key = f"feedme:websocket:queue:{user_id}"
                redis_messages = await self._await_redis(
                    self.redis_client.lrange(redis_key, 0, -1)
                )
                ...
                for redis_msg in redis_messages:
                    try:
                        ...
                        await self._send_message_safe(
                            websocket, message_data["message"]
                        )
                ...
                if delivered_messages:
                    for msg in delivered_messages:
                        await self._await_redis(
                            self.redis_client.lrem(redis_key, 1, msg)
                        )
                    logger.debug(
                        f"Delivered and removed {len(delivered_messages)} messages from Redis for user {user_id}"
                    )
        ...
        if user_id in self.message_queues:
            ...
            if successfully_delivered:
                logger.debug(
                    f"Delivered and removed {len(successfully_delivered)} messages from memory for user {user_id}"
                )
    ...
    async def cleanup_stale_connections(self):
        ...
        for room_id, room in list(self.rooms.items()):
            ...
            for websocket, connection_info in room.connections.items():
                ...
                if (
                    connection_info.last_activity
                    and connection_info.last_activity < stale_cutoff
                ):
