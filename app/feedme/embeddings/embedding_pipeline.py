...
class FeedMeEmbeddingPipeline:
    ...
    def __init__(
        ...
        content_length_normalization_factor: Optional[float] = None,
    ):
        ...
        self.content_length_normalization_factor = (
            content_length_normalization_factor
            or self.DEFAULT_CONTENT_LENGTH_NORMALIZATION_FACTOR
        )
        ...
        try:
            for i, pair in enumerate(qa_pairs):
                ...
                if self.enable_quality_scoring:
                    pair["embedding_quality_score"] = self._assess_embedding_quality(
                        pair
                    )
                ...
                if "combined_embedding" in pair:
                    ...
                    if embedding_dim != self.dimension:
                        logger.warning(
                            f"Embedding dimension mismatch: expected {self.dimension}, got {embedding_dim}"
                        )
                        # Pad or truncate to expected dimension
                        if embedding_dim < self.dimension:
                            pair["combined_embedding"].extend(
                                [0.0] * (self.dimension - embedding_dim)
                            )
                        else:
                            pair["combined_embedding"] = pair["combined_embedding"][
                                : self.dimension
                            ]
            ...
            logger.info(
                f"Generated embeddings for {len(qa_pairs)} Q&A pairs in {time.time() - start_time:.2f}s"
            )
            return qa_pairs
    ...
    def _generate_semantic_embedding(self, pair: Dict[str, Any]) -> List[float]:
        ...
        semantic_embedding = self.model.encode(semantic_text, normalize_embeddings=True)
    ...
    def _assess_embedding_quality(self, pair: Dict[str, Any]) -> float:
        ...
        try:
            ...
            norm_score = (float(q_norm) + float(a_norm)) / 2.0
            norm_score = min(1.0, norm_score)

            # Content length factor
            content_length = len(pair.get("question_text", "")) + len(
                pair.get("answer_text", "")
            )
            length_score = min(
                1.0, float(content_length) / self.content_length_normalization_factor
            )
