...
class WordCorpusLoader:
    ...
    def _check_nltk_availability(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning("NLTK words corpus not downloaded. Falling back to config file.")
                return False
    ...
    def _load_config_words(self) -> Set[str]:
        ...
        try:
            ...
            logger.error(f"Failed to load word configuration from {self.config_path}: {e}")
            # Return minimal fallback word set
            return {
                'the', 'be', 'to', 'of', 'and', 'a', 'in', 'that', 'have', 'i',
                'it', 'for', 'not', 'on', 'with', 'he', 'as', 'you', 'do', 'at',
                'email', 'password', 'login', 'account', 'user', 'help', 'error'
            }
    ...
    def get_word_corpus(self, use_nltk: bool = True, min_word_length: int = 2) -> Set[str]:
        """
...
def get_word_corpus(use_nltk: bool = True, min_word_length: int = 2) -> Set[str]:
    ...
    return _default_loader.get_word_corpus(use_nltk=use_nltk, min_word_length=min_word_length)
