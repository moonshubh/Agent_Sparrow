...
class ConfidenceWeights:
    ...
    def validate(self) -> bool:
        ...
        if not (0.95 <= total_weight <= 1.05):
            logger.warning(
                f"Confidence weights sum to {total_weight:.3f}, should be close to 1.0"
            )
        ...
        for field_name, value in asdict(self).items():
            if field_name.endswith("_ratio") and not field_name.endswith("_scale"):
                if not (0.0 <= value <= 1.0):
                    logger.warning(
                        f"Weight {field_name}={value} should be between 0.0 and 1.0"
                    )
                    return False
...
class OCRConfigManager:
    ...
    def _create_default_config(self) -> OCRFallbackConfig:
        ...
        return OCRFallbackConfig(confidence_weights=ConfidenceWeights())
        ...
        for env_var, (field_path, sub_field) in env_mappings.items():
            ...
            if env_value is not None:
                try:
                    ...
                    if "WEIGHT" in env_var or field_path in [
                        "low_text_confidence",
                        "special_char_ratio_threshold",
                    ]:
                    ...
                    logger.debug(
                        f"Override {field_path}.{sub_field or ''} with {env_value}"
                    )
