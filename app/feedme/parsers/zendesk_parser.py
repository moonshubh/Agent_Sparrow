...
class ZendeskTicketParser:
    ...
    def parse_zendesk_pdf(self, text: str) -> Optional[ZendeskTicket]:
        ...
        try:
            ...
            self.logger.info(
                f"Successfully parsed Zendesk ticket #{ticket.ticket_number} with {len(conversations)} conversations"
            )
            return ticket
    ...
    def _clean_conversation_content(self, content: str) -> str:
        ...
        content = re.sub(
            r"_{10,}.*?From:.*?$", "", content, flags=re.DOTALL | re.MULTILINE
        )

        # Remove "Support Software by Zendesk" footer
        content = re.sub(
            r"Support Software by Zendesk.*$", "", content, flags=re.MULTILINE
        )

        # Remove URLs that are just ticket references
        content = re.sub(
            r"https://mailbird\.zendesk\.com/tickets/\d+/print.*$",
            "",
            content,
            flags=re.MULTILINE,
        )
