...
try:
    from app.feedme.ocr import EasyOCREngine, OCRResult, ProcessingQuality, create_ocr_engine
    OCR_AVAILABLE = True
except ImportError:
    OCR_AVAILABLE = False
...
try:
    from app.feedme.parsers.zendesk_parser import ZendeskTicketParser, ZendeskTicket
...
class PDFParseResult:
    ...
    error_message: Optional[str] = None
    warnings: List[str] = None
    extraction_method: Optional[str] = None  # Added: Track extraction method (direct, ocr_fallback)
    quality_metrics: Optional[Dict[str, float]] = None  # Added: Quality metrics from OCR
...
class EnhancedPDFParser(PDFParser):
        ...
        if ZENDESK_PARSER_AVAILABLE:
            try:
                ...
                if ticket and ticket.conversations:
                    ...
                    logger.info(f"Extracted {len(threads)} conversations using Zendesk parser")
                    return threads
            except Exception as e:
                logger.warning(f"Zendesk parser failed, falling back to pattern matching: {e}")
    ...
    async def parse_pdf(self, file_content: bytes) -> PDFParseResult:
        ...
        try:
            ...
            if is_extraction_successful:
                ...
                logger.info(f"Direct PDF parsing successful: {direct_result.metadata.pages} pages, "
            ...
            if self.enable_ocr and self.ocr_engine:
                ...
                ocr_result = await self._parse_with_ocr(file_content, direct_result.metadata)
                
                if ocr_result.success:
                    # Combine OCR result with PDF metadata
                    enhanced_result = self._combine_ocr_with_metadata(ocr_result, direct_result)
                    processing_time_ms = (asyncio.get_event_loop().time() - start_time) * 1000
                    enhanced_result.processing_time_ms = processing_time_ms
                    logger.info(f"OCR fallback successful: {enhanced_result.total_chars} chars extracted")
            ...
            if not is_extraction_successful:
                direct_result.warnings.append("Low text extraction quality detected - document may be scanned")
            
            logger.warning(f"PDF parsing completed with warnings: {direct_result.total_chars} chars extracted")
            return direct_result
    ...
    def _assess_extraction_quality(self, result: PDFParseResult) -> bool:
        ...
        if result.total_chars < self.min_text_length:
            logger.info(f"Low text volume: {result.total_chars} chars < {self.min_text_length} minimum")
            return False
        ...
        if result.metadata.pages > 0:
            ...
            if chars_per_page < self.min_text_density:
                logger.info(f"Low text density: {chars_per_page:.1f} chars/page < {self.min_text_density} minimum")
                return False
        ...
        if words:
            ...
            if single_char_ratio > 0.5:  # More than 50% single character "words"
                logger.info(f"High single character ratio: {single_char_ratio:.2f} > 0.5 threshold")
                return False
        
        logger.info("Direct text extraction quality assessment: PASSED")
        return True
    
    async def _parse_with_ocr(self, pdf_bytes: bytes, pdf_metadata: PDFMetadata) -> OCRResult:
        """
    ...
    def _combine_ocr_with_metadata(self, ocr_result: OCRResult, direct_result: PDFParseResult) -> PDFParseResult:
        """
        ...
        warnings.append(f"Text extracted via OCR (confidence: {ocr_result.confidence_score:.2f})")
