# mypy: ignore-errors
"""
Enhanced PDF Parser for FeedMe System

This module provides advanced PDF parsing capabilities with OCR fallback
for extracting text from both native PDFs and scanned documents.
Integrates with EasyOCR for comprehensive text extraction.
"""
...
try:
    from app.feedme.ocr import OCRResult, create_ocr_engine

    OCR_AVAILABLE = True
except ImportError:
    OCR_AVAILABLE = False

    # Define OCRResult fallback
    @dataclass
    class OCRResult:
        """Fallback OCRResult when OCR is not available"""
...
# Import Zendesk parser
try:
    from app.feedme.parsers.zendesk_parser import ZendeskTicketParser

    ZENDESK_PARSER_AVAILABLE = True
except ImportError:
    ZENDESK_PARSER_AVAILABLE = False
    logger.warning("Zendesk parser not available")
...
class PDFParseResult:
    ...
    extraction_method: Optional[str] = (
        None  # Added: Track extraction method (direct, ocr_fallback)
    )
    quality_metrics: Optional[Dict[str, float]] = (
        None  # Added: Quality metrics from OCR
    )
...
class EnhancedPDFParser(PDFParser):
    """
    Enhanced PDF parser with OCR fallback and conversation detection

    Features:
    - Direct text extraction from native PDFs
    - OCR fallback for scanned documents using EasyOCR
    - Gemma 3 text enhancement
    - Unified text output for canvas display
    """
    ...
    def __init__(
        ...
        if self.enable_ocr:
            try:
                self.ocr_engine = create_ocr_engine(
                    languages=ocr_languages or ["en"], quality=ocr_quality, use_gpu=True
                )
        ...
        self.min_text_length = 100  # Minimum chars for successful direct extraction
        self.min_text_density = 10  # Minimum chars per page for quality check

    async def parse_pdf_with_conversations(
        self, content: bytes
    ) -> PDFConversationResult:
    ...
    def _extract_conversations_from_page(
    ...
    def _extract_email_threads(
        ...
        for match in re.finditer(email_pattern, text, re.DOTALL | re.IGNORECASE):
            from_name, from_email, to_name, to_email, subject, date_str, content = (
                match.groups()
            )
    ...
    def _extract_zendesk_threads(
        self, text: str, page_num: int
    ) -> List[ConversationThread]:
        """Extract Zendesk ticket conversation threads using specialized parser"""
        threads = []

        # Try specialized Zendesk parser first
        if ZENDESK_PARSER_AVAILABLE:
            try:
                zendesk_parser = ZendeskTicketParser()
                ticket = zendesk_parser.parse_zendesk_pdf(text)

                if ticket and ticket.conversations:
                    for conv in ticket.conversations:
                        thread = ConversationThread(
                            ...
                            page_number=page_num,
                            thread_id=f"zendesk_{ticket.ticket_number}_{conv.get('timestamp', '')}",
                            confidence=conv.get("confidence", 0.9),
                            metadata={
                    ...
                    logger.info(
                        f"Extracted {len(threads)} conversations using Zendesk parser"
                    )
                    return threads
            except Exception as e:
                logger.warning(
                    f"Zendesk parser failed, falling back to pattern matching: {e}"
                )
    ...
    def _determine_role_from_email(self, email: str) -> str:
        ...
        if any(
            domain in email_lower for domain in ["support", "help", "mailbird", "agent"]
        ):
            return "agent"
        return "customer"

    def _determine_role_from_context(
        self, name: str, role_hint: str, content: str
    ) -> str:
        """Determine role based on context clues"""
        # Check role hint first
        if role_hint and any(
            hint in role_hint.lower() for hint in ["support", "agent", "mailbird"]
        ):
            return "agent"
    ...
    def _merge_conversation_threads(
        ...
        sorted_conversations = sorted(
            conversations, key=lambda x: (x.timestamp or datetime.min, x.page_number)
        )
        ...
        for conv in sorted_conversations:
            if self._should_merge_with_current(conv, current_thread):
                ...
                current_thread.confidence = max(
                    current_thread.confidence, conv.confidence
                )
            else:
                # Start new thread
                if current_thread:
    ...
    def _should_merge_with_current(
        self, conv: ConversationThread, current: Optional[ConversationThread]
    ) -> bool:
        ...
        if (
            conv.speaker == current.speaker
            and conv.timestamp
            and current.timestamp
            and abs((conv.timestamp - current.timestamp).total_seconds()) < 300
        ):  # 5 minutes
            return True
        ...
        if (
            conv.speaker == current.speaker
            and conv.page_number == current.page_number + 1
        ):
            return True
    ...
    async def parse_pdf(self, file_content: bytes) -> PDFParseResult:
        ...
        try:
            ...
            is_extraction_successful = self._assess_extraction_quality(direct_result)

            if is_extraction_successful:
                processing_time_ms = (
                    asyncio.get_event_loop().time() - start_time
                ) * 1000
                direct_result.processing_time_ms = processing_time_ms
                logger.info(
                    f"Direct PDF parsing successful: {direct_result.metadata.pages} pages, "
                    f"{direct_result.total_chars} chars in {processing_time_ms:.2f}ms"
                )
                return direct_result

            # Step 2: Fallback to OCR if direct extraction failed and OCR is available
            if self.enable_ocr and self.ocr_engine:
                logger.info("Direct extraction insufficient, attempting OCR fallback")
                ocr_result = await self._parse_with_ocr(
                    file_content, direct_result.metadata
                )

                if ocr_result.success:
                    # Combine OCR result with PDF metadata
                    enhanced_result = self._combine_ocr_with_metadata(
                        ocr_result, direct_result
                    )
                    processing_time_ms = (
                        asyncio.get_event_loop().time() - start_time
                    ) * 1000
                    ...
                    logger.info(
                        f"OCR fallback successful: {enhanced_result.total_chars} chars extracted"
                    )
                    return enhanced_result
                else:
                    warnings.append("OCR fallback failed")

            # Step 3: Return direct result with warnings if OCR is not available or failed
            processing_time_ms = (asyncio.get_event_loop().time() - start_time) * 1000
            direct_result.processing_time_ms = processing_time_ms
            direct_result.warnings.extend(warnings)

            if not is_extraction_successful:
                direct_result.warnings.append(
                    "Low text extraction quality detected - document may be scanned"
                )

            logger.warning(
                f"PDF parsing completed with warnings: {direct_result.total_chars} chars extracted"
            )
            return direct_result
        ...
        try:
            with pdfplumber.open(io.BytesIO(file_content)) as pdf:
                ...
                if len(pdf.pages) > self.max_pages:
                    warnings.append(
                        f"PDF has {len(pdf.pages)} pages, processing first {self.max_pages}"
                    )
                ...
                return PDFParseResult(
                    ...
                    extraction_method="direct",  # Mark as direct extraction
                )
    ...
    def _extract_page_content(self, page, page_number: int) -> PageContent:
        ...
        try:
            ...
            return PageContent(
                page_number=page_number, text="[Page extraction failed]", char_count=0
            )
    ...
    def _create_error_result(
        ...
        return PDFParseResult(
            ...
            extraction_method="failed",  # Mark as failed extraction
        )

    def _assess_extraction_quality(self, result: PDFParseResult) -> bool:
        """
        Assess if direct text extraction was successful

        Args:
            result: Result from direct PDF text extraction

        Returns:
            True if extraction quality is sufficient, False if OCR fallback needed
        """
        if not result.success:
            return False

        # Check total text length
        if result.total_chars < self.min_text_length:
            logger.info(
                f"Low text volume: {result.total_chars} chars < {self.min_text_length} minimum"
            )
            return False

        # Check text density per page
        if result.metadata.pages > 0:
            chars_per_page = result.total_chars / result.metadata.pages
            if chars_per_page < self.min_text_density:
                logger.info(
                    f"Low text density: {chars_per_page:.1f} chars/page < {self.min_text_density} minimum"
                )
                return False

        # Check for signs of poor extraction (lots of individual characters)
        words = result.total_text.split()
        if words:
            single_char_words = sum(1 for word in words if len(word.strip()) == 1)
            single_char_ratio = single_char_words / len(words)
            if single_char_ratio > 0.5:  # More than 50% single character "words"
                logger.info(
                    f"High single character ratio: {single_char_ratio:.2f} > 0.5 threshold"
                )
                return False

        logger.info("Direct text extraction quality assessment: PASSED")
        return True

    async def _parse_with_ocr(
        self, pdf_bytes: bytes, pdf_metadata: PDFMetadata
    ) -> OCRResult:
        """
        Parse PDF using OCR engine

        Args:
            pdf_bytes: PDF file content
            pdf_metadata: Metadata from direct PDF parsing
        ...
        try:
            ...
            ocr_result = await self.ocr_engine.process_pdf(
                pdf_bytes=pdf_bytes, enhance_with_gemma=True, metadata=metadata
            )
    ...
    def _combine_ocr_with_metadata(
        self, ocr_result: OCRResult, direct_result: PDFParseResult
    ) -> PDFParseResult:
        """
        Combine OCR text extraction with PDF metadata

        Args:
            ocr_result: Result from OCR processing
            direct_result: Result from direct PDF parsing (for metadata)

        Returns:
            PDFParseResult with OCR text and PDF metadata
        """
        # Create pages from OCR result
        pages = []
        for page_info in ocr_result.page_results:
            page = PageContent(
                page_number=page_info.get("page_number", 1),
                text=page_info.get("text", ""),
                char_count=len(page_info.get("text", "")),
                has_tables=False,  # OCR doesn't detect tables
            )
            pages.append(page)

        # Enhance metadata with OCR information
        enhanced_metadata = direct_result.metadata
        enhanced_metadata.encrypted = False  # OCR was successful, so PDF is accessible

        # Add OCR-specific warnings
        warnings = direct_result.warnings.copy()
        warnings.append(
            f"Text extracted via OCR (confidence: {ocr_result.confidence_score:.2f})"
        )
        warnings.append(f"Language detected: {ocr_result.language_detected}")
        ...
        return PDFParseResult(
            ...
            total_text=ocr_result.extracted_text,
            total_chars=len(ocr_result.extracted_text),
            processing_time_ms=ocr_result.processing_time * 1000,
            warnings=warnings,
