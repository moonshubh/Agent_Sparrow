...
class ExtractionConfig:
    ...
    enable_fallback_processing: bool = (
        True  # Enable pattern-based fallback when AI fails
    )

    # Configurable conversation markers for generic content
    conversation_markers: List[str] = field(
    ...
    def can_make_request(self, estimated_tokens: int) -> Tuple[bool, float]:
        ...
        if minute_limit and minute_requests >= minute_limit:
            ...
            if oldest_minute is not None:
                request_delay = max(
                    request_delay, (oldest_minute + self.config.rate_limit_window) - now
                )
...
class GeminiExtractionEngine:
    ...
    def __init__(
        self, api_key: Optional[str] = None, config: Optional[ExtractionConfig] = None
    ):
        ...
        logger.info(
            f"Initialized AI extraction engine with model: {self.config.model_name}"
        )
        logger.info(
    ...
    async def extract_conversations(
        ...
        if (
            ...
            or len(html_content) > self.config.max_chunk_size
        ):
            logger.info(
                f"Large content detected ({estimated_tokens} estimated tokens), using intelligent chunking strategy"
            )
        ...
        try:
            ...
            if response is None:
                logger.info(
                    "AI model rate limited, falling back to pattern-based extraction"
                )
            ...
            logger.info(
                f"Extracted {len(high_quality_pairs)} high-quality Q&A pairs from {len(extracted_pairs)} total"
            )
            ...
            if self.config.enable_fallback_processing:
                logger.info(
                    "AI extraction failed, falling back to pattern-based extraction"
                )
                return await self._fallback_pattern_extraction(html_content)
            return []
...
"""
    ...
    async def _extract_with_retry(self, prompt: str) -> Any:
        ...
        for attempt in range(self.config.max_retries):
            ...
            can_proceed, delay_needed = self.rate_limiter.can_make_request(
                estimated_tokens
            )

            if not can_proceed:
                logger.info(
                    f"Rate limit would be exceeded, waiting {delay_needed:.1f}s before request"
                )
            ...
            try:
                ...
                if hasattr(e, "details") and e.details:
                    for detail in e.details:
                        if hasattr(detail, "reason") and detail.reason in [
                            "RESOURCE_EXHAUSTED",
                            "RATE_LIMIT_EXCEEDED",
                            "QUOTA_EXCEEDED",
                        ]:
                ...
                if is_rate_limit_error:
                    ...
                    wait_time = min(
                        base_delay * delay_multiplier, self.config.rate_limit_max_delay
                    )

                    if attempt < self.config.max_retries - 1:
                        logger.warning(
                            f"Rate limit/quota exceeded, waiting {wait_time:.1f}s before retry (attempt {attempt + 1})"
                        )
                    ...
                    if attempt < self.config.max_retries - 1:
                        ...
                        logger.warning(
                            f"Resource exhausted, retrying in {wait_time}s (attempt {attempt + 1})"
                        )
                        await asyncio.sleep(wait_time)
                        continue
                    else:
                ...
                if attempt < self.config.max_retries - 1:
                    ...
                    logger.error(
                        f"Extraction failed after {self.config.max_retries} attempts: {e}"
                    )
                    raise
            ...
            for pair in parsed_data:
                if self._validate_qa_pair(pair):
                    ...
                    standardized_pair = {
                        ...
                        "quality_score": float(
                            pair.get("quality_score", pair.get("confidence_score", 0.5))
                        ),
    ...
    async def intelligent_chunk_and_extract(
        self, html_content: str, metadata: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Intelligent chunking complete: {len(merged_pairs)} unique pairs from {len(all_pairs)} total, ~{total_tokens_used} tokens used"
        )
        return merged_pairs
            ...
            for marker in conversation_markers:
                ...
                if elements and len(elements) > 1:
                    ...
                    if len(chunks) > 1 and all(
                        len(c) > self.config.min_chunk_size for c in chunks
                    ):
                        logger.info(
                            f"Created {len(chunks)} conversation-aware chunks using {marker}"
                        )
                        return chunks

            # If conversation-aware chunking failed, fall back to character chunking
            logger.info(
                "Conversation-aware chunking failed, using character-based chunking"
            )
            return self._simple_character_chunks(html_content)

        except Exception as e:
            logger.warning(
                f"Intelligent chunking failed: {e}, falling back to simple chunking"
            )
            return self._simple_character_chunks(html_content)
    ...
    def _merge_and_deduplicate_pairs(
    ...
    async def _fallback_pattern_extraction(
        ...
        try:
            ...
            logger.info(
                f"Fallback extraction completed: {len(standardized_pairs)} pairs extracted"
            )
    ...
    async def chunk_and_extract(
        self, html_content: str, chunk_size: int = 50000, concurrency_limit: int = 3
    ) -> List[Dict[str, Any]]:
    ...
    def _merge_chunk_results(
        ...
        logger.info(
            f"Merged {len(all_pairs)} pairs into {len(unique_pairs)} unique pairs"
        )
        return unique_pairs
    ...
    async def _extract_from_pdf(
        self, pdf_base64_content: str, metadata: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"PDF parsed successfully: {conversation_result.metadata.pages} pages, {len(conversation_result.conversations)} conversations"
            )
            ...
            if ocr_processor.should_use_ocr(conversation_result.raw_text, ""):
                ...
                enhanced_text = await ocr_processor.enhance_text_with_ocr(
                    conversation_result.raw_text, pdf_bytes
                )
                conversation_result.raw_text = enhanced_text

            # Extract Q&A pairs from PDF conversations
            if conversation_result.conversations:
                qa_pairs = await self._extract_from_pdf_conversations(
                    conversation_result, metadata
                )
            else:
                # Fallback to basic text extraction
                qa_pairs = await self._extract_qa_from_pdf_text(
                    conversation_result, metadata
                )
    ...
    async def _extract_from_pdf_conversations(
        self, pdf_result, metadata: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        conversation_threads = self._group_conversations_by_thread(
            pdf_result.conversations
        )
    ...
    def _group_conversations_by_thread(self, conversations) -> List[List]:
        ...
        for conv in sorted(
            conversations, key=lambda x: (x.page_number, x.timestamp or datetime.min)
        ):
    ...
    def _should_start_new_thread(self, conv, current_thread) -> bool:
        ...
        if (
            conv.page_number != last_conv.page_number
            and conv.speaker != last_conv.speaker
        ):
            return True

        return False

    async def _extract_qa_from_thread(
        self, thread, metadata: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response:
                ...
                for qa_pair in qa_pairs:
                    ...
                    qa_pair["page_range"] = (
                        f"{thread[0].page_number}-{thread[-1].page_number}"
                    )
            ...
            logger.warning(
                f"AI extraction failed for thread {thread[0].thread_id}: {e}"
            )
...
"""
            ...
            if conv.role == "customer" and i + 1 < len(thread):
                ...
                if next_conv.role == "agent":
                    qa_pair = {
                        ...
                        "context_after": (
                            thread[i + 2].content if i + 2 < len(thread) else ""
                        ),
    ...
    def _deduplicate_qa_pairs_semantic(
        ...
        for pair in qa_pairs:
            ...
            for seen_q in seen_questions:
                ...
                if question_words and seen_words:
                    ...
                    similarity = len(question_words.intersection(seen_words)) / len(
                        question_words.union(seen_words)
                    )
        ...
        logger.info(
            f"Semantic deduplication: {len(qa_pairs)} → {len(unique_pairs)} pairs"
        )
        return unique_pairs

    async def _extract_qa_from_pdf_text(
        self, parse_result, metadata: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
...
"""
    ...
    async def _chunked_pdf_extraction(
        self, parse_result, metadata: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Chunked PDF extraction complete: {len(all_qa_pairs)} Q&A pairs from {len(parse_result.pages)} pages"
        )
        return all_qa_pairs
...
"""
    ...
    async def _fallback_pdf_pattern_extraction(
        self, pdf_text: str
    ) -> List[Dict[str, Any]]:
    ...
    def extract_conversations_sync(
        ...
        try:
            ...
            try:
                result = loop.run_until_complete(
                    self.extract_conversations(content, metadata)
                )
                return result
            finally:
                loop.close()
    ...
    async def summarize_conversation(
        self, conversation_text: str, max_length: int = 500, focus: str = "key_points"
    ) -> Dict[str, Any]:
        """
...
{{
    ...
    "agent_performance": {{
        ...
        "problem_solving": "excellent/good/needs_improvement"
            ...
            return {"success": True, "data": summary_data, "confidence": 0.9}
    ...
    async def analyze_conversation_batch(
        self, conversations: List[Dict[str, Any]], analysis_type: str = "patterns"
    ) -> Dict[str, Any]:
        """
...
class FallbackExtractor:
    ...
    def __init__(self):
        self.qa_patterns = [
            ...
            (
                re.compile(
                    r"(?i)(how\s+(?:do|can)\s+i|what\s+(?:is|are)|why\s+(?:is|are|do)|when\s+(?:do|should))"
                ),
                "question",
            ),
            # Problem statements
            (
                re.compile(
                    r"(?i)(problem|issue|error|trouble|can\'t|cannot|unable|won\'t|doesn\'t)"
                ),
                "question",
            ),
            # Solution patterns
            (
                re.compile(r"(?i)(try|follow|go\s+to|click|select|configure|set)"),
                "answer",
            ),
        ]
