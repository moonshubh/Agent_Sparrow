...
from .schemas import (
    SearchEvent, AnalyticsInsights, UsageMetrics, 
    SearchBehaviorMetrics, UserBehaviorAnalytics,
    QueryPattern, TrendAnalysis, AnomalyDetection
)
...
class UsageAnalytics:
    ...
    async def _update_real_time_metrics(self, event: SearchEvent) -> None:
        ...
        pipe.lpush(f"feedme:metrics:response_times:{current_hour}", event.response_time_ms)
        pipe.ltrim(f"feedme:metrics:response_times:{current_hour}", 0, 999)  # Keep last 1000
        ...
        pipe.expire(f"feedme:metrics:search_type:{event.search_type}:{current_hour}", 86400)
    ...
    async def _get_search_patterns_by_segment(
        ...
        return {
            'expert': {'avg_queries_per_session': 8.5, 'preferred_search_type': 'hybrid'},
            'intermediate': {'avg_queries_per_session': 4.2, 'preferred_search_type': 'vector'},
            'beginner': {'avg_queries_per_session': 2.1, 'preferred_search_type': 'text'}
        }
