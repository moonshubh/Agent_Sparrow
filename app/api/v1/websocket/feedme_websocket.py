...
async def get_current_user_from_token(token: Optional[str] = Query(None)) -> str:
    ...
    try:
        ...
        if "demo-signature" in token:
            ...
            if len(parts) == 3:
                try:
                    ...
                    if user_id:
                        logger.info(
                            f"Demo authentication successful for user: {email or user_id}"
                        )
        ...
        try:
            ...
            if not JWT_SECRET:
                logger.warning("JWT secret key is not configured")
                raise HTTPException(
            ...
            if JWT_SECRET in ("your-secret-key-here", "change-this-in-production"):
                logger.warning(
                    "JWT secret is using default value - this is insecure for production!"
                )
...
async def get_user_permissions(user_id: str) -> List[str]:
    ...
    try:
        ...
        role_permissions = {
            ...
            "viewer": ["processing:read", "approval:read"],
            "user": ["processing:read"],
        }
...
async def websocket_processing_updates(
    websocket: WebSocket, conversation_id: int, token: Optional[str] = Query(None)
):
    """
    ...
    try:
        ...
        logger.info(
            f"Processing WebSocket connected: user={user_id}, conversation={conversation_id}"
        )
        ...
        try:
            ...
            logger.info(
                f"Processing WebSocket disconnected: user={user_id}, conversation={conversation_id}"
            )
    ...
    try:
        ...
        for room_id, room in realtime_manager.rooms.items():
            room_info = RoomInfo(
                ...
                unique_users=len(
                    set(conn.user_id for conn in room.connections.values())
                ),
        ...
        return RoomListResponse(
            rooms=rooms, total_rooms=len(rooms), total_connections=total_connections
        )
...
async def get_room_users(
    room_id: str, current_user: str = Depends(get_current_user_from_token)
):
    ...
    try:
        ...
        return {"room_id": room_id, "user_count": len(user_list), "users": user_list}
...
async def _handle_client_message(
    websocket: WebSocket, message: dict, user_id: str, room_id: str
):
    ...
    try:
        ...
        if message_type == "ping":
            ...
            logger.info(
                f"User {user_id} subscribed to {subscription_type} in room {room_id}"
            )
            ...
            subscription_type = message.get("subscription_type")
            logger.info(
                f"User {user_id} unsubscribed from {subscription_type} in room {room_id}"
            )
        ...
        # Send error response
        await websocket.send_json(
            {"type": "error", "error": "Failed to process message", "details": str(e)}
        )
...
async def notify_processing_update(update: ProcessingUpdate):
    ...
    try:
        ...
        logger.debug(
            f"Broadcasted processing update for conversation {update.conversation_id}"
        )
    except Exception as e:
...
async def notify_approval_update(update: ApprovalUpdate):
    ...
    try:
        ...
        logger.debug(
            f"Broadcasted approval update for temp example {update.temp_example_id}"
        )
    except Exception as e:
