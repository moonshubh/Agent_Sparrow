...
async def list_conversations(
    ...
    folder_id: Optional[int] = Query(
        UNASSIGNED_FOLDER_ID, description="Filter by folder ID"
    ),
    include_all: bool = Query(
        False, description="Include all conversations regardless of folder"
    ),
):
...
@router.get(
    "/conversations/{conversation_id}/examples", response_model=ExampleListResponse
)
...
async def update_example(example_id: int, updates: ExampleUpdate):
    ...
    try:
        ...
        updated_example = await client.update_example(
            example_id=example_id, update_data=update_data
        )
...
async def get_formatted_qa_content(conversation_id: int):
    ...
    try:
        ...
        formatted_lines = [
            ...
            "",
            f"**Total Examples:** {len(active_examples)}",
            "",
        ]
        ...
        for i, example in enumerate(active_examples, 1):
            ...
            if example.get("context_before"):
                formatted_lines.extend(
                    ["**Context Before:**", example["context_before"], ""]
                )
            ...
            if example.get("context_after"):
                formatted_lines.extend(
                    ["**Context After:**", example["context_after"], ""]
                )
