...
from app.core.rate_limiting.memory_limiter import InMemoryRateLimiter
...
async def _enforce_rate_limit(key: str, limit: int, window_seconds: int) -> None:
    allowed, _, reset_time = await rate_limiter.check_rate_limit(
        key, limit, window_seconds
    )
    if allowed:
        return
    retry_after = str(reset_time or window_seconds)
    raise HTTPException(
...
async def create_temp_example(
    ...
    try:
        # Apply rate limiting
        await _enforce_rate_limit(f"create_temp_example:{current_user}", 10, 60)
...
async def get_temp_example(
    ...
    try:
        result = await workflow_engine.get_temp_example(temp_example_id)

        if not result:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Temp example not found"
            )
...
async def list_temp_examples(
    approval_status: Optional[ApprovalState] = Query(
        None, alias="status", description="Filter by approval status"
    ),
    assigned_reviewer: Optional[str] = Query(
        None, description="Filter by assigned reviewer"
    ),
    ...
    try:
        ...
        if approval_status:
            filters["approval_status"] = approval_status.value
        ...
        result = await workflow_engine.list_temp_examples(
            filters=filters,
            page=page,
            page_size=page_size,
        )
...
async def update_temp_example(
    ...
    try:
        ...
        existing = await workflow_engine.get_temp_example(temp_example_id)
        if not existing:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Temp example not found"
            )
        ...
        updated = await workflow_engine.update_temp_example(
...
async def assign_reviewer(
    temp_example_id: int,
    assignment_data: Optional[Dict[str, Optional[str]]] = None,
    workflow_engine: ApprovalWorkflowEngine = Depends(get_workflow_engine),
    current_user: str = Depends(get_current_user),
):
    """
    ...
    try:
        ...
        updated = await workflow_engine.get_temp_example(temp_example_id)
        if not updated:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Temp example not found"
            )
        ...
        logger.error(
            f"Failed to assign reviewer to temp example {temp_example_id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def process_approval_decision(
    ...
    try:
        ...
        await _enforce_rate_limit(f"approval_decision:{current_user}", 30, 60)

        # Create decision object
        decision = ApprovalDecision(
            temp_example_id=temp_example_id, reviewer_id=current_user, **decision_data
        )

        result = await workflow_engine.process_approval_decision(
            temp_example_id, decision
        )
...
async def process_bulk_approval(
    ...
    try:
        ...
        await _enforce_rate_limit(f"bulk_approval:{current_user}", 5, 60)
        ...
        return JSONResponse(status_code=response_status, content=result.model_dump())
    ...
    try:
        ...
        if metrics.total_pending > 50:
            recommendations.append(
                "High number of pending reviews. Consider adding more reviewers."
            )

        if metrics.approval_rate < 0.8:
            recommendations.append(
                "Low approval rate detected. Review extraction quality or thresholds."
            )

        if metrics.auto_approval_rate < 0.5:
            recommendations.append(
                "Low auto-approval rate. Consider adjusting confidence thresholds."
            )
...
@router.post("/maintenance/cleanup", summary="Clean up old rejected items")
async def cleanup_old_rejected_items(
    ...
    try:
        ...
        cleanup_count = await workflow_engine.cleanup_old_rejected_items(cutoff_date)
...
@router.get("/health", summary="Health check for approval system")
async def health_check(
    ...
    try:
        ...
        try:
            await workflow_engine.get_approval_metrics(
