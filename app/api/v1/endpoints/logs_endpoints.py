...
async def analyze_logs(
    request: LogAnalysisRequest, user_id: str = Depends(get_current_user_id)
):
    ...
    try:
        try:
            ...
            await log_analysis_request_middleware(
                fake_request, user_id, log_body, file_name="analysis.log"
            )
            ...
            raise HTTPException(
                status_code=500, detail="Failed to prepare log analysis request"
            )
        ...
        if isinstance(result, dict) and "error" in result:
            raise HTTPException(
                status_code=500, detail=result.get("message", "Log analysis failed")
            )
        ...
        if isinstance(result, dict):
            ...
            response_dict["trace_id"] = (
                result.get("trace_id") or request.trace_id or session_id
            )
        else:
            raise HTTPException(
                status_code=500,
                detail="Log analysis agent did not return a valid report.",
            )
        try:
            return LogAnalysisV2Response(**response_dict)
        except Exception as validation_error:
            logger.warning(
                f"LogAnalysisV2Response validation error: {validation_error}"
            )
            fallback = {
                "overall_summary": response_dict.get(
                ...
                "issue_summary_by_severity": response_dict.get(
                ...
                "monitoring_recommendations": response_dict.get(
                    "monitoring_recommendations", []
                ),
                "automated_remediation_available": response_dict.get(
...
async def log_analysis_stream_generator(
    ...
    try:
        if user_id:
            ...
            try:
                await log_analysis_request_middleware(
                    fake_request, user_id, log_content, file_name="analysis.log"
                )
                concurrent_slot_acquired = True
        ...
        yield format_sse_data(
            {
                "type": "step",
                "data": {
                    "type": "Starting Analysis",
                    "description": "Initializing log analysis...",
                    "status": "in-progress",
                },
            }
        )
            ...
            yield format_sse_data(
                {
                    ...
                    "data": {
                        "type": "Parsing",
                        "description": "Parsing log entries...",
                        "status": "in-progress",
                    },
                }
            )
            ...
            yield format_sse_data(
                {
                    ...
                    "data": {
                        "type": "Analyzing",
                        "description": "Analyzing patterns and issues...",
                        "status": "complete",
                    },
                }
            )
...
async def add_session_insight(
    session_id: str, insight: dict, user_id: str = Depends(get_current_user_id)
):
    success = await log_analysis_session_manager.add_insight(
        user_id, session_id, insight
    )
    if not success:
        raise HTTPException(status_code=404, detail="Session not found")
...
async def get_log_analysis_rate_limits(user_id: str = Depends(get_current_user_id)):
    ...
    requests_last_minute = len(
        [r for r in user_requests if now - r < timedelta(minutes=1)]
    )
    ...
    return {
        ...
        "remaining": {
            "per_minute": max(
                0,
                LOG_ANALYSIS_RATE_LIMITS["requests_per_minute"] - requests_last_minute,
            ),
            "per_hour": max(
                0, LOG_ANALYSIS_RATE_LIMITS["requests_per_hour"] - requests_last_hour
            ),
            "per_day": max(
                0, LOG_ANALYSIS_RATE_LIMITS["requests_per_day"] - requests_today
            ),
            "concurrent": max(
                0, LOG_ANALYSIS_RATE_LIMITS["max_concurrent_analyses"] - concurrent
            ),
        },
    }
