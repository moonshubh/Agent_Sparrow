...
from app.tools.research_tools import (
    get_research_tools,
)  # To get TavilySearchTool instance
from app.db.embedding import utils as embedding_utils  # For internal KB search
from app.db.embedding.utils import SearchResult
...
class WebSearchQuery(BaseModel):
    query: str = Field(
        ..., description="The search query for the web.", min_length=3, max_length=500
    )
...
try:
    ...
    for tool_func in available_tools:
        ...
        if (
            hasattr(tool_func, "__qualname__")
            and "TavilySearchTool.search" in tool_func.__qualname__
        ):
            tavily_search_func = tool_func
            break
    if not tavily_search_func:
        logger.warning(
            "Tavily search tool function not found via get_research_tools(). Web search might be disabled."
        )

        # Fallback to a dummy to prevent startup crash, but log error
        def dummy_tavily_search(query: str, max_results: int = 5) -> dict:
            logger.error(
                "Dummy Tavily search called because actual tool was not found."
            )
    ...
    logger.error(
        f"Failed to import or initialize research tools: {error_message}. Web search will be disabled."
    )

    def dummy_tavily_search_on_import_error(
        query: str, max_results: int = 5, _error: str = error_message
    ) -> dict:
        return {"urls": [], "error": f"Tavily tool import failed: {_error}"}
...
async def web_search_endpoint(
    ...
    if not tavily_search_func or (
        hasattr(tavily_search_func, "__name__")
        and "dummy" in tavily_search_func.__name__
    ):
        logger.error("Tavily search tool is not available or not properly configured.")
        raise HTTPException(
            status_code=503, detail="Web search service is currently unavailable."
        )
    ...
    try:
        ...
        raw_results = tavily_search_func(
            query=search_params.query, max_results=search_params.max_results
        )

        processed_results: List[WebSearchResultItem] = []
        if isinstance(raw_results, dict) and "error" in raw_results:
            logger.error(
                f"Tavily search failed for query '{search_params.query}': {raw_results['error']}"
            )
            ...
            isinstance(raw_results, dict) and "urls" in raw_results
        ):  # Current tool output
            for url_item in raw_results["urls"]:
                if isinstance(url_item, str):  # If it's just a list of URL strings
                    ...
                    url_item, dict
                ):  # If it's a list of dicts with more details
        ...
        logger.exception(
            f"Error during web search for query '{search_params.query}': {e}"
        )
        raise HTTPException(
            status_code=500,
            detail=f"An unexpected error occurred during web search: {str(e)}",
        )


@router.post(
    "/internal-search", response_model=InternalSearchResults, tags=["Search Tools"]
)
async def internal_kb_search_endpoint(
    ...
    try:
        top_k = search_params.top_k or 5
        similar_docs = embedding_utils.find_similar_documents(
            query=search_params.query, top_k=top_k
        )
        results: List[SearchResult] = []
        for doc in similar_docs:
            if isinstance(doc, SearchResult):
                ...
                results.append(SearchResult.model_validate(doc))
        return InternalSearchResults(query=search_params.query, results=results)
    except Exception as e:
        logger.exception(
            f"Error during internal KB search for query '{search_params.query}': {e}"
        )
        # Check if it's a DB connection issue or embedding model issue from underlying function
        if "database" in str(e).lower() or "connection" in str(e).lower():
            raise HTTPException(
                status_code=503,
                detail="Internal search service is temporarily unavailable due to a database issue.",
            )
        elif "GEMINI_API_KEY" in str(e) or "embedding model" in str(e).lower():
            raise HTTPException(
                status_code=503,
                detail="Internal search service is temporarily unavailable due to an embedding model configuration issue.",
            )
        raise HTTPException(
            status_code=500,
            detail=f"An unexpected error occurred during internal search: {str(e)}",
        )
