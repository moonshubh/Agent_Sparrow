...
def verify_internal_access(
    request: Request, x_internal_token: Optional[str] = Header(None)
):
...
async def update_api_key(
    ...
    if update_request.api_key is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="api_key is required when updating an API key",
        )
...
async def validate_api_key_format(
    ...
    return service.validate_api_key_format(
        api_key_type=validate_request.api_key_type, api_key=validate_request.api_key
    )
    ...
    try:
        if api_key_type == APIKeyType.GEMINI:
            ...
            message = (
                "Gemini key is valid"
                if success
                else f"Gemini API responded {resp.status_code}"
            )
            ...
            message = (
                "OpenAI key is valid"
                if success
                else f"OpenAI API responded {resp.status_code}"
            )
            ...
            message = (
                "Tavily key is valid"
                if success
                else f"Tavily API responded {resp.status_code}"
            )
        else:
...
async def debug_api_key_usage(
    ...
    try:
        response = await service.supabase._exec(
            lambda: service.supabase.client.table("user_api_keys")
    ...
    return {
        "user_id": user_id,
        "api_key_source": (
            "user" if using_user_key else ("fallback" if using_fallback else "none")
        ),
        ...
        "user_key_details": (
            {
                ...
                "created_at": (
                    user_key_info.get("created_at") if user_key_info else None
                ),
                "last_used_at": (
                    user_key_info.get("last_used_at") if user_key_info else None
                ),
            }
            if user_key_info
            else None
        ...
        "actual_key_preview": (
            mask_key(gemini_key) if gemini_key else "No key available"
        ),
        "decision_logic": (
            "Using user's frontend-configured API key"
            if using_user_key
            else (
                "Using Railway environment fallback key"
                if using_fallback
