...
from app.feedme.approval_workflow import (
...
class ApprovalPreviewResponse(BaseModel):
    ...
    conversations: List[Dict[str, Any]] = Field(
        ..., description="Conversations pending approval"
    )
    total_count: int = Field(
        ..., description="Total number of conversations pending approval"
    )
...
class TextApprovalRequest(BaseModel):
    ...
    def validate_action_requirements(self):
        ...
        if (
            self.action in [ApprovalAction.REJECT, ApprovalAction.REQUEST_REPROCESS]
...
class BulkApprovalRequest(BaseModel):
    ...
    conversation_ids: List[int] = Field(
        ..., min_items=1, max_items=50, description="Conversation IDs to process"
    )
    action: ApprovalAction = Field(
        ..., description="Action to apply to all conversations"
    )
    reviewer_id: str = Field(..., min_length=1, description="ID of the reviewer")
    notes: Optional[str] = Field(
        None, description="Notes to apply to all conversations"
    )
...
class BulkApprovalResponse(BaseModel):
    ...
    failed: List[Dict[str, Any]] = Field(
        ..., description="Failed operations with details"
    )
...
async def get_pending_approvals(
    limit: int = Query(
        default=20, ge=1, le=100, description="Number of conversations to return"
    ),
    offset: int = Query(default=0, ge=0, description="Offset for pagination"),
    processing_method: Optional[ProcessingMethod] = Query(
        default=None, description="Filter by processing method"
    ),
    ...
    try:
        result = await workflow.get_pending_approvals(
            limit=limit, offset=offset, processing_method=processing_method
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve pending approvals"
        )
...
async def get_conversation_approval_preview(
    ...
    try:
        ...
        if workflow.supabase_client:
            conversation = await workflow.supabase_client.get_conversation_by_id(
                conversation_id
            )
        else:
...
@router.post(
    "/conversation/{conversation_id}/decide", response_model=TextApprovalResponse
)
async def make_approval_decision(
    ...
    try:
        ...
        return TextApprovalResponse(
            ...
            or result.get("rejected_at")
            or result.get("requested_at"),
        ...
        raise HTTPException(
            status_code=500, detail="Failed to process approval decision"
        )
    ...
    try:
        ...
        for conversation_id in request.conversation_ids:
            try:
                ...
                # Process the decision
                await workflow.process_approval_decision(decision)
                successful.append(conversation_id)

            except Exception as e:
                failed.append({"conversation_id": conversation_id, "error": str(e)})
                logger.error(
                    f"Failed to process conversation {conversation_id} in bulk operation: {e}"
                )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to perform bulk approval operation"
        )
...
async def get_approval_statistics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve approval statistics"
        )
...
async def get_approval_queue_summary(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve approval queue summary"
        )
