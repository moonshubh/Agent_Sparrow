...
# Initialize router with security configurations
router = APIRouter(
    prefix="/feedme",
    tags=["feedme"],
    responses={
...
def get_feedme_supabase_client():
    ...
    if _supabase_client is None:
        try:
            ...
            # Return None to indicate Supabase is not available
            return None
...
class _SupabaseClientProxy:
    def __getattr__(self, name):
        ...
        if not client:
            raise RuntimeError("Supabase client not initialized. Please configure environment variables.")
        return getattr(client, name)

# Use the proxy for backwards compatibility
supabase_client = _SupabaseClientProxy()

# Gemini usage endpoint (FeedMe-specific, backed by in-process tracker)
@router.get("/gemini-usage", tags=["FeedMe"])
async def get_gemini_usage():
    try:
        ...
        return {
            "status": "ok",
            "day": info.get("day"),
            "daily_used": info.get("daily_used"),
            "daily_limit": info.get("daily_limit"),
            "rpm_limit": info.get("rpm_limit"),
            "calls_in_window": info.get("calls_in_window"),
            "window_seconds_remaining": info.get("window_seconds_remaining"),
            "utilization": info.get("utilization"),
        }
    except Exception as e:
        logger.error(f"Failed to get Gemini usage: {e}")
        raise HTTPException(status_code=500, detail="Failed to get Gemini usage")

@router.get("/embedding-usage", tags=["FeedMe"])
async def get_embedding_usage():
    try:
        from app.feedme.rate_limiting.gemini_tracker import get_embed_tracker_info
        info = get_embed_tracker_info(
        ...
        return {
            ...
            "tokens_in_window": info.get("tokens_in_window"),
            "token_window_seconds_remaining": info.get("token_window_seconds_remaining"),
            "utilization": info.get("utilization"),
        }
    except Exception as e:
        logger.error(f"Failed to get Embedding usage: {e}")
        raise HTTPException(status_code=500, detail="Failed to get Embedding usage")


# Helper: hard-disable legacy Q&A example flows
def _examples_gone():
    if not settings.feedme_examples_enabled:
        raise HTTPException(
            status_code=410,
            detail="Legacy Q&A example flows have been removed. Use the unified text workflow (extracted_text)."
        )
...
async def upload_transcript(
    ...
    if transcript_file:
        ...
        await transcript_file.seek(0)  # Reset for any other operations
        ...
        # Sanitize filename
        original_filename = SecurityValidator.sanitize_filename(transcript_file.filename)
        
        # Validate file size (redundant but kept for clarity)
...
async def list_conversations(
    ...
    if client is None:
        raise HTTPException(
            status_code=503, 
            detail="FeedMe service is temporarily unavailable. Please check Supabase configuration."
        )
...
@router.get("/conversations/{conversation_id}", response_model=ConversationDetailResponse, tags=["FeedMe"])
async def get_conversation(conversation_id: int):
    """
    ...
    Returns:
        ConversationDetailResponse: The conversation details including metadata and processing status.
    
    Raises:
        HTTPException: If the FeedMe service is disabled (503) or conversation not found (404).
    """
    ...
    try:
        ...
        if not client:
            raise HTTPException(
                status_code=503,
                detail="FeedMe service unavailable. Please configure Supabase environment variables."
            )
        
        # Retrieve conversation using Supabase
...
async def list_conversation_examples(
    ...
    if not settings.feedme_enabled:
        raise HTTPException(status_code=503, detail="FeedMe service is currently disabled")
    # Examples are deprecated/removed
    _examples_gone()
...
async def get_formatted_qa_content(conversation_id: int):
    ...
    try:
        # Unified text workflow: return extracted_text if available; fallback to raw_transcript
        formatted = conversation.extracted_text or conversation.raw_transcript or ""
        return {
            "formatted_content": formatted,
            "total_examples": 0,
            "content_type": "raw_transcript",
            "raw_transcript": conversation.raw_transcript or "",
            "message": "Q&A examples are deprecated. Showing unified text content."
        }
    except Exception as e:
        ...
        return {
            ...
            "total_examples": 0,
            "content_type": "raw_transcript",
            "message": "Showing raw transcript."
        }
...
@router.get("/analytics/pdf-storage", response_model=Dict[str, Any], tags=["FeedMe"])
async def get_pdf_storage_analytics():
    """
    Get PDF storage analytics and cleanup metrics
    
    Returns:
        Dict containing PDF storage statistics:
        - pending_cleanup: Number of approved PDFs awaiting cleanup
        - cleaned_count: Number of PDFs already cleaned
        - total_mb_freed: Total MB freed by cleanup
        - avg_pdf_size_mb: Average PDF size in MB
        - total_pdf_conversations: Total number of PDF-based conversations
    """
    if not settings.feedme_enabled:
        raise HTTPException(status_code=503, detail="FeedMe service is currently disabled")
    ...
    try:
        ...
        if result.data and len(result.data) > 0:
            ...
            return {
                "pending_cleanup": analytics.get('pending_cleanup', 0),
                "cleaned_count": analytics.get('cleaned_count', 0),
                "total_mb_freed": float(analytics.get('total_mb_freed', 0)),
                "avg_pdf_size_mb": float(analytics.get('avg_pdf_size_mb', 0)),
                "total_pdf_conversations": analytics.get('total_pdf_conversations', 0),
...
@router.post("/cleanup/pdfs/batch", response_model=Dict[str, Any], tags=["FeedMe"])
async def trigger_pdf_cleanup_batch(limit: int = 100):
    """
    Trigger batch cleanup of approved PDFs
    
    Args:
        limit: Maximum number of PDFs to clean in this batch (default: 100)
...
async def manually_process_conversation(conversation_id: int):
    ...
    try:
        ...
        try:
            ...
            # Queue the task
            # Use the provided processed_by (uploader) as user context for API key resolution
            result = process_transcript.delay(conversation_id, processed_by)
            task_id = result.id
...
async def approve_conversation(conversation_id: int, approval_request: ApprovalRequest):
    ...
    try:
        ...
        # Legacy examples are removed; no per-example approval is performed
        logger.info(f"Conversation {conversation_id} approved by {approval_request.approved_by} (examples flow disabled)")
        ...
        try:
            ...
            logger.info(f"Scheduled PDF cleanup for conversation {conversation_id}")
        except Exception as e:
            logger.warning(f"Failed to schedule PDF cleanup for conversation {conversation_id}: {e}")
            # Don't fail the approval if cleanup scheduling fails
...
async def reject_conversation(conversation_id: int, rejection_request: RejectionRequest):
    ...
    try:
        ...
        # Legacy examples are removed; no per-example rejection is performed
        logger.info(f"Conversation {conversation_id} rejected by {rejection_request.rejected_by} (examples flow disabled)")
...
# Removed duplicate PUT endpoint - using POST /folders/assign instead
...
async def update_folder_supabase(
    ...
    try:
        ...
        logger.info(f"Successfully updated folder {folder_id} via Supabase")
        
        return {
            "folder": FeedMeFolder(**updated_folder),
            "supabase_sync": "completed",
            "message": f"Successfully updated folder '{updated_folder['name']}'."
        }
