...
@router.get("/tools/tavily/self-test", response_model=TavilySelfTestResponse, tags=["Search Tools"])
async def tavily_self_test(user_id: str = Depends(get_current_user_id)):
    ...
    try:
        ...
        from app.tools.user_research_tools import tavily_web_search
        ...
        return TavilySelfTestResponse(ok=False, status=res.get("status"), error=res.get("error") or "no_results")
