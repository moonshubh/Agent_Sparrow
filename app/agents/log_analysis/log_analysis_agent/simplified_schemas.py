...
class LogSection(BaseModel):
    ...
    relevance_score: float = Field(
        description="How relevant this section is to the question",
        ge=0.0,
        le=1.0
    )
...
class SimplifiedSolution(BaseModel):
    ...
    steps: List[str] = Field(
        description="Action steps to resolve the issue",
        min_length=1
    )
...
class SimplifiedLogAnalysisRequest(BaseModel):
    ...
    question: Optional[str] = Field(default=None, description="Specific question about the logs")
...
class SimplifiedLogAnalysisOutput(BaseModel):
    ...
    overall_summary: str = Field(description="Direct answer to the user's question or executive summary")
    
    # Backward compatibility fields for AI SDK
    health_status: str = Field(default="Analyzed", description="System health status")
    priority_concerns: List[str] = Field(default_factory=list, description="Key concerns from the logs")
    identified_issues: List[SimplifiedIssue] = Field(default_factory=list, description="Issues found")
    proposed_solutions: List[SimplifiedSolution] = Field(default_factory=list, description="Solutions")
    
    # Question-driven fields
    question: Optional[str] = Field(default=None, description="The question that was answered")
    relevant_log_sections: Optional[List[LogSection]] = Field(default=None, description="Log sections relevant to the question")
    confidence_level: float = Field(default=0.8, description="Confidence in the analysis (0-1)")
    
    # Metadata
    analysis_timestamp: str = Field(default_factory=lambda: datetime.utcnow().isoformat())
    trace_id: Optional[str] = Field(default=None, description="Request trace ID")
    analysis_method: str = Field(default="simplified", description="Analysis method used")
