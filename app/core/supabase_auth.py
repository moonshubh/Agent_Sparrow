...
class SupabaseAuthClient:
    ...
    def __init__(self):
        ...
        if settings.supabase_jwt_secret:
            ...
            logger.warning(
                "SUPABASE_JWT_SECRET not found in environment, using fallback JWT_SECRET_KEY"
            )
        ...
        try:
            ...
            if auth_response.user:
                ...
                await self._audit_log(
                    ...
                    event_type="sign_up",
                    event_details={"email": email, "metadata": metadata},
        ...
        try:
            ...
            if auth_response.session:
                ...
                await self._store_session(
                    auth_response.session, ip_address=ip_address, user_agent=user_agent
                )
    ...
    async def sign_in_with_oauth(
        ...
        try:
            ...
            error_msg = (
                f"OAuth initialization failed: {self._sanitize_auth_error(str(e))}"
            )
            ...
            if self.jwt_secret and self.jwt_secret != "change-this-in-production":
                try:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"JWT verification without audience also failed: {e2}"
                        )
                        return None
                ...
                logger.error(
                    "CRITICAL: JWT secret not configured properly! JWT_SECRET value: "
    ...
    async def get_user_from_token(self, jwt_token: str) -> Optional[User]:
        """
    ...
    async def _is_session_revoked(self, token: str) -> bool:
        ...
        try:
            ...
            response = (
                await supabase.table("auth_sessions")
                .select("revoked_at")
                .eq("session_token", token)
                .single()
                .execute()
