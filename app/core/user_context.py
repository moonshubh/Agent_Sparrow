...
class UserContext:
    ...
    async def get_api_key(
        self, api_key_type: APIKeyType, fallback_env_var: Optional[str] = None
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            logger.error(
                f"Error retrieving API key {api_key_type} for user {self.user_id}: {e}"
            )
...
def require_user_context() -> UserContext:
    ...
    if user_context is None:
        raise RuntimeError(
            "No user context set. Ensure authentication is properly configured."
        )
    return user_context
...
async def create_user_context_from_user_id(
    ...
    """
    return UserContext(
        user_id=user_id, email=email, full_name=full_name, metadata=metadata
    )
