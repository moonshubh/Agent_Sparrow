...
class Settings(BaseSettings):
    ...
    openrouter_app_name: str = Field(
        default="Agent Sparrow", alias="OPENROUTER_APP_NAME"
    )
    ...
    minimax_api_host: str = Field(
        default="https://api.minimax.io", alias="MINIMAX_API_HOST"
    )
    ...
    langsmith_tracing_enabled: bool = Field(
        default=False, alias="LANGSMITH_TRACING_ENABLED"
    )
    ...
    session_store_max_sessions: int = Field(
        default=100, alias="SESSION_STORE_MAX_SESSIONS"
    )
    session_store_default_ttl: int = Field(
        default=3600, alias="SESSION_STORE_DEFAULT_TTL"
    )
    session_store_cleanup_interval: int = Field(
        default=300, alias="SESSION_STORE_CLEANUP_INTERVAL"
    )

    # Workspace retention (Phase 1: keep N sessions per user)
    workspace_max_sessions_per_user: int = Field(
        default=10, alias="WORKSPACE_MAX_SESSIONS_PER_USER"
    )
    workspace_prune_sessions_enabled: bool = Field(
        default=True, alias="WORKSPACE_PRUNE_SESSIONS_ENABLED"
    )

    # Subagent workspace bridge (Phase 1)
    subagent_workspace_bridge_enabled: bool = Field(
        default=True, alias="SUBAGENT_WORKSPACE_BRIDGE_ENABLED"
    )
    subagent_report_read_limit_chars: int = Field(
        default=20000, alias="SUBAGENT_REPORT_READ_LIMIT_CHARS"
    )
    subagent_context_capsule_max_chars: int = Field(
        default=12000, alias="SUBAGENT_CONTEXT_CAPSULE_MAX_CHARS"
    )
    ...
    enable_grounding_search: bool = Field(
        default=False, alias="ENABLE_GROUNDING_SEARCH"
    )
    ...
    firecrawl_default_max_age_ms: int = Field(
        default=172800000, alias="FIRECRAWL_DEFAULT_MAX_AGE_MS"
    )  # 48 hours
    firecrawl_default_timeout_sec: float = Field(
        default=60.0, alias="FIRECRAWL_DEFAULT_TIMEOUT_SEC"
    )
    firecrawl_rate_limit_rpm: int = Field(default=60, alias="FIRECRAWL_RATE_LIMIT_RPM")

    # Enhanced Tavily Configuration
    tavily_api_key: Optional[str] = Field(default=None, alias="TAVILY_API_KEY")
    tavily_default_search_depth: str = Field(
        default="advanced", alias="TAVILY_DEFAULT_SEARCH_DEPTH"
    )
    tavily_default_max_results: int = Field(
        default=10, alias="TAVILY_DEFAULT_MAX_RESULTS"
    )
    ...
    use_enhanced_log_analysis: bool = Field(
        default=True, alias="USE_ENHANCED_LOG_ANALYSIS"
    )
    primary_agent_temperature: float = Field(
        default=0.2, alias="PRIMARY_AGENT_TEMPERATURE"
    )
    primary_agent_thinking_budget: Optional[int] = Field(
        default=None, alias="THINKING_BUDGET"
    )
    primary_agent_formatting: str = Field(
        default="natural", alias="PRIMARY_AGENT_FORMATTING"
    )
    primary_agent_quality_level: str = Field(
        default="balanced", alias="PRIMARY_AGENT_QUALITY_LEVEL"
    )
    primary_agent_prompt_version: str = Field(
        default="v10", alias="PRIMARY_AGENT_PROMPT_VERSION"
    )
    ...
    enable_grounded_responses: bool = Field(
        default=True, alias="ENABLE_GROUNDED_RESPONSES"
    )
    primary_agent_min_kb_relevance: float = Field(
        default=0.65, alias="PRIMARY_AGENT_MIN_KB_RELEVANCE"
    )
    primary_agent_min_kb_results: int = Field(
        default=1, alias="PRIMARY_AGENT_MIN_KB_RESULTS"
    )
    ...
    checkpointer_max_overflow: int = Field(
        default=10, alias="CHECKPOINTER_MAX_OVERFLOW"
    )
    graph_viz_export_enabled: bool = Field(
        default=False, alias="ENABLE_GRAPH_VIZ_EXPORT"
    )
    ...
    feedme_max_examples_per_conversation: int = Field(
        default=20, alias="FEEDME_MAX_EXAMPLES_PER_CONVERSATION"
    )
    feedme_embedding_batch_size: int = Field(
        default=10, alias="FEEDME_EMBEDDING_BATCH_SIZE"
    )
    feedme_max_retrieval_results: int = Field(
        default=3, alias="FEEDME_MAX_RETRIEVAL_RESULTS"
    )
    ...
    feedme_pdf_processing_timeout: int = Field(
        default=30, alias="FEEDME_PDF_PROCESSING_TIMEOUT"
    )
    feedme_pdf_concurrent_limit: int = Field(
        default=5, alias="FEEDME_PDF_CONCURRENT_LIMIT"
    )

    # Enhanced PDF Processing Configuration
    feedme_max_tokens_per_chunk: int = Field(
        default=8000, alias="FEEDME_MAX_TOKENS_PER_CHUNK"
    )
    feedme_chunk_overlap_tokens: int = Field(
        default=500, alias="FEEDME_CHUNK_OVERLAP_TOKENS"
    )

    # OCR Processing Configuration
    feedme_ocr_enabled: bool = Field(default=True, alias="FEEDME_OCR_ENABLED")
    feedme_ocr_confidence_threshold: float = Field(
        default=0.7, alias="FEEDME_OCR_CONFIDENCE_THRESHOLD"
    )

    # Quality Control Configuration
    feedme_similarity_threshold: float = Field(
        default=0.7, alias="FEEDME_SIMILARITY_THRESHOLD"
    )
    feedme_confidence_threshold: float = Field(
        default=0.7, alias="FEEDME_CONFIDENCE_THRESHOLD"
    )
    ...
    feedme_celery_broker: str = Field(
        default="redis://localhost:6379/1", alias="FEEDME_CELERY_BROKER"
    )
    feedme_result_backend: str = Field(
        default="redis://localhost:6379/2", alias="FEEDME_RESULT_BACKEND"
    )
    ...
    feedme_quality_threshold: float = Field(
        default=0.7, alias="FEEDME_QUALITY_THRESHOLD"
    )

    # Reasoning Engine Configuration
    reasoning_enable_chain_of_thought: bool = Field(
        default=True, alias="REASONING_ENABLE_CHAIN_OF_THOUGHT"
    )
    reasoning_enable_problem_solving: bool = Field(
        default=True, alias="REASONING_ENABLE_PROBLEM_SOLVING"
    )
    reasoning_enable_tool_intelligence: bool = Field(
        default=True, alias="REASONING_ENABLE_TOOL_INTELLIGENCE"
    )
    reasoning_enable_quality_assessment: bool = Field(
        default=True, alias="REASONING_ENABLE_QUALITY_ASSESSMENT"
    )
    reasoning_enable_reasoning_transparency: bool = Field(
        default=True, alias="REASONING_ENABLE_REASONING_TRANSPARENCY"
    )
    reasoning_debug_mode: bool = Field(default=False, alias="REASONING_DEBUG_MODE")
    reasoning_enable_thinking_trace: bool = Field(
        default=True, alias="ENABLE_THINKING_TRACE"
    )
    ...
    log_analysis_use_optimized_analysis: bool = Field(
        default=True, alias="USE_OPTIMIZED_ANALYSIS"
    )
    log_analysis_optimization_threshold_lines: int = Field(
        default=500, alias="OPTIMIZATION_THRESHOLD_LINES"
    )
    log_analysis_enable_ml_pattern_discovery: bool = Field(
        default=True, alias="ENABLE_ML_PATTERN_DISCOVERY"
    )
    log_analysis_enable_predictive_analysis: bool = Field(
        default=True, alias="ENABLE_PREDICTIVE_ANALYSIS"
    )
    log_analysis_enable_correlation_analysis: bool = Field(
        default=True, alias="ENABLE_CORRELATION_ANALYSIS"
    )
    log_analysis_enable_automated_remediation: bool = Field(
        default=False, alias="ENABLE_AUTOMATED_REMEDIATION"
    )
    log_analysis_enable_cross_platform_support: bool = Field(
        default=True, alias="ENABLE_CROSS_PLATFORM_SUPPORT"
    )
    log_analysis_enable_multi_language_support: bool = Field(
        default=True, alias="ENABLE_MULTI_LANGUAGE_SUPPORT"
    )
    log_analysis_enable_dependency_analysis: bool = Field(
        default=True, alias="ENABLE_DEPENDENCY_ANALYSIS"
    )
    log_analysis_enable_edge_case_handling: bool = Field(
        default=True, alias="ENABLE_EDGE_CASE_HANDLING"
    )

    # Performance and Optimization Settings
    log_analysis_max_chunk_size: int = Field(
        default=5000, alias="LOG_ANALYSIS_MAX_CHUNK_SIZE"
    )
    log_analysis_max_context_chars: int = Field(
        default=15000, alias="LOG_ANALYSIS_MAX_CONTEXT_CHARS"
    )
    log_analysis_parallel_chunks: int = Field(
        default=3, alias="LOG_ANALYSIS_PARALLEL_CHUNKS"
    )
    log_analysis_cache_ttl_hours: int = Field(
        default=1, alias="LOG_ANALYSIS_CACHE_TTL_HOURS"
    )
    log_analysis_ml_confidence_threshold: float = Field(
        default=0.85, alias="ML_CONFIDENCE_THRESHOLD"
    )
    log_analysis_correlation_threshold: float = Field(
        default=0.7, alias="CORRELATION_THRESHOLD"
    )

    # Testing and Validation Settings
    log_analysis_enable_comprehensive_testing: bool = Field(
        default=False, alias="ENABLE_COMPREHENSIVE_TESTING"
    )
    log_analysis_test_framework_enabled: bool = Field(
        default=False, alias="TEST_FRAMEWORK_ENABLED"
    )
    log_analysis_validation_strict_mode: bool = Field(
        default=False, alias="VALIDATION_STRICT_MODE"
    )
    ...
    chat_session_cleanup_days: int = Field(
        default=30, alias="CHAT_SESSION_CLEANUP_DAYS"
    )
    chat_enable_message_history: bool = Field(
        default=True, alias="CHAT_ENABLE_MESSAGE_HISTORY"
    )
    ...
    memory_collection_primary: str = Field(
        default="mem_primary", alias="MEMORY_COLLECTION_PRIMARY"
    )
    memory_collection_logs: str = Field(
        default="mem_logs", alias="MEMORY_COLLECTION_LOGS"
    )
    ...
    enable_memory_ui_capture: bool = Field(
        default=False, alias="ENABLE_MEMORY_UI_CAPTURE"
    )
    enable_memory_ui_retrieval: bool = Field(
        default=False, alias="ENABLE_MEMORY_UI_RETRIEVAL"
    )
    ...
    primary_agent_daily_budget: int = Field(
        default=500, alias="PRIMARY_AGENT_DAILY_BUDGET"
    )
    ...
    api_key_encryption_secret: Optional[str] = Field(
        default=None, alias="API_KEY_ENCRYPTION_SECRET"
    )

    # Authentication
    skip_auth: bool = Field(default=False, alias="SKIP_AUTH")
    # Use a valid UUID for development (this is a v4 UUID)
    development_user_id: str = Field(
        default="00000000-0000-0000-0000-000000000000", alias="DEVELOPMENT_USER_ID"
    )
    ...
    enable_api_key_endpoints: bool = Field(
        default=True, alias="ENABLE_API_KEY_ENDPOINTS"
    )
    force_production_security: bool = Field(
        default=True, alias="FORCE_PRODUCTION_SECURITY"
    )
    ...
    production_domains: List[str] = Field(
        default=["supabase.co"], alias="PRODUCTION_DOMAINS"
    )
    ...
    zendesk_signing_secret: Optional[str] = Field(
        default=None, alias="ZENDESK_SIGNING_SECRET"
    )
    ...
    zendesk_windows_brand_ids: List[str] = Field(
        default_factory=list, alias="ZENDESK_WINDOWS_BRAND_IDS"
    )
    zendesk_dry_run: bool = Field(default=True, alias="ZENDESK_DRY_RUN")
    zendesk_poll_interval_sec: int = Field(
        default=60, alias="ZENDESK_POLL_INTERVAL_SEC"
    )
    ...
    zendesk_monthly_api_budget: int = Field(
        default=0, alias="ZENDESK_MONTHLY_API_BUDGET"
    )
    zendesk_gemini_daily_limit: int = Field(
        default=380, alias="ZENDESK_GEMINI_DAILY_LIMIT"
    )
    zendesk_max_retries: int = Field(default=5, alias="ZENDESK_MAX_RETRIES")
    zendesk_queue_retention_days: int = Field(
        default=30, alias="ZENDESK_QUEUE_RETENTION_DAYS"
    )
    # Optional: prefetch web context (Firecrawl/Tavily) when internal sources are insufficient
    zendesk_web_prefetch_enabled: bool = Field(
        default=True, alias="ZENDESK_WEB_PREFETCH_ENABLED"
    )
    zendesk_web_prefetch_pages: int = Field(
        default=3, alias="ZENDESK_WEB_PREFETCH_PAGES"
    )
    zendesk_firecrawl_enhanced_enabled: bool = Field(
        default=True, alias="ZENDESK_FIRECRAWL_ENHANCED_ENABLED"
    )
    ...
    zendesk_firecrawl_support_pages: int = Field(
        default=3, alias="ZENDESK_FIRECRAWL_SUPPORT_PAGES"
    )
    zendesk_firecrawl_support_screenshots: bool = Field(
        default=False, alias="ZENDESK_FIRECRAWL_SUPPORT_SCREENSHOTS"
    )
    # Query Agent (Phase 1 context engineering)
    zendesk_query_agent_enabled: bool = Field(
        default=True, alias="ZENDESK_QUERY_AGENT_ENABLED"
    )
    zendesk_query_agent_model: str = Field(
        default="gemini-2.5-flash", alias="ZENDESK_QUERY_AGENT_MODEL"
    )
    zendesk_query_reformulation_max_attempts: int = Field(
        default=2, alias="ZENDESK_QUERY_REFORMULATION_MAX_ATTEMPTS"
    )
    zendesk_query_confidence_threshold: float = Field(
        default=0.6, alias="ZENDESK_QUERY_CONFIDENCE_THRESHOLD"
    )
    zendesk_query_expansion_count: int = Field(
        default=2, alias="ZENDESK_QUERY_EXPANSION_COUNT"
    )
    ...
    zendesk_internal_retrieval_min_relevance: float = Field(
        default=0.35, alias="ZENDESK_INTERNAL_RETRIEVAL_MIN_RELEVANCE"
    )
    zendesk_internal_retrieval_max_per_source: int = Field(
        default=3, alias="ZENDESK_INTERNAL_RETRIEVAL_MAX_PER_SOURCE"
    )
    zendesk_macro_min_relevance: float = Field(
        default=0.55, alias="ZENDESK_MACRO_MIN_RELEVANCE"
    )
    zendesk_feedme_min_relevance: float = Field(
        default=0.45, alias="ZENDESK_FEEDME_MIN_RELEVANCE"
    )
    ...
    zendesk_rerank_model: str = Field(
        default="gemini-2.5-flash", alias="ZENDESK_RERANK_MODEL"
    )
    zendesk_rerank_timeout_sec: int = Field(
        default=180, alias="ZENDESK_RERANK_TIMEOUT_SEC"
    )
    zendesk_clash_detection_enabled: bool = Field(
        default=True, alias="ZENDESK_CLASH_DETECTION_ENABLED"
    )
    zendesk_clash_resolution: str = Field(
        default="prefer_newer", alias="ZENDESK_CLASH_RESOLUTION"
    )
    zendesk_context_budget_tokens: int = Field(
        default=8000, alias="ZENDESK_CONTEXT_BUDGET_TOKENS"
    )
    zendesk_confidence_high_threshold: float = Field(
        default=0.8, alias="ZENDESK_CONFIDENCE_HIGH_THRESHOLD"
    )
    zendesk_confidence_low_threshold: float = Field(
        default=0.6, alias="ZENDESK_CONFIDENCE_LOW_THRESHOLD"
    )
    ...
    zendesk_drift_guard_enabled: bool = Field(
        default=True, alias="ZENDESK_DRIFT_GUARD_ENABLED"
    )
    ...
    zendesk_issue_pattern_max_hits: int = Field(
        default=5, alias="ZENDESK_ISSUE_PATTERN_MAX_HITS"
    )
    zendesk_issue_pattern_min_similarity: float = Field(
        default=0.62, alias="ZENDESK_ISSUE_PATTERN_MIN_SIMILARITY"
    )
    zendesk_issue_pattern_learning_enabled: bool = Field(
        default=True, alias="ZENDESK_ISSUE_PATTERN_LEARNING_ENABLED"
    )
    zendesk_playbook_learning_enabled: bool = Field(
        default=True, alias="ZENDESK_PLAYBOOK_LEARNING_ENABLED"
    )
    zendesk_nature_field_id: Optional[str] = Field(
        default=None, alias="ZENDESK_NATURE_FIELD_ID"
    )
    zendesk_nature_field_ids: List[str] = Field(
        default_factory=list, alias="ZENDESK_NATURE_FIELD_IDS"
    )
    zendesk_nature_category_map: Dict[str, str] = Field(
        default_factory=dict, alias="ZENDESK_NATURE_CATEGORY_MAP"
    )
    zendesk_nature_require_field: bool = Field(
        default=True, alias="ZENDESK_NATURE_REQUIRE_FIELD"
    )
    ...
    zendesk_format_engine: str = Field(
        default="markdown_v2", alias="ZENDESK_FORMAT_ENGINE"
    )
    ...
    def validate_zendesk_format_engine(cls, value: str) -> str:
        ...
        if normalized not in {"legacy", "markdown_v2"}:
            raise ValueError(
                "zendesk_format_engine must be one of: legacy, markdown_v2"
            )
        return normalized
    ...
    def validate_zendesk_drift_guard_strictness(cls, value: str) -> str:
        ...
        if normalized not in {"low", "medium", "high"}:
            raise ValueError(
                "zendesk_drift_guard_strictness must be low, medium, or high"
            )
        return normalized

    @field_validator(
        "zendesk_feedme_slice_window_before", "zendesk_feedme_slice_window_after"
    )
    @classmethod
    ...
    def validate_feedme_max_pdf_size_mb(cls, v: int) -> int:
        ...
        if v < MIN_PDF_SIZE_MB:
            raise ValueError(
                f"feedme_max_pdf_size_mb must be at least {MIN_PDF_SIZE_MB} MB"
            )

        if v > MAX_PDF_SIZE_MB:
            raise ValueError(
                f"feedme_max_pdf_size_mb must not exceed {MAX_PDF_SIZE_MB} MB for server capabilities"
            )
    ...
    def validate_api_key_encryption_secret(cls, v: str) -> str:
        ...
        if len(v.encode("utf-8")) < 32:
            raise ValueError(
                "api_key_encryption_secret must be at least 32 bytes long when UTF-8 encoded"
            )
        return v
    ...
    def validate_primary_agent_min_kb_relevance(cls, v: float) -> float:
        if v < 0.0 or v > 1.0:
            raise ValueError(
                "primary_agent_min_kb_relevance must be between 0.0 and 1.0"
            )
        return v
    ...
    @classmethod
    def validate_primary_agent_thinking_budget(
        ...
        if value < -1:
            raise ValueError(
                "primary_agent_thinking_budget must be -1 (dynamic) or non-negative"
            )
        return value
    ...
    def validate_primary_agent_formatting(cls, value: str) -> str:
        ...
        if normalized not in {"natural", "strict", "lean"}:
            raise ValueError(
                "primary_agent_formatting must be one of: natural, strict, lean"
            )
        return normalized
    ...
    @field_validator(
        "zendesk_rpm_limit",
        "zendesk_import_rpm_limit",
        "zendesk_gemini_daily_limit",
        "zendesk_max_retries",
    )
    @classmethod
    ...
    def validate_zendesk_monthly_budget(cls, v: int) -> int:
        if v < 0:
            raise ValueError(
                "zendesk_monthly_api_budget must be zero or a positive integer"
            )
        return v
    ...
    def validate_zendesk_relevance_thresholds(cls, v: float) -> float:
        if v < 0.0 or v > 1.0:
            raise ValueError(
                "zendesk_* relevance threshold must be between 0.0 and 1.0"
            )
        return v
    ...
    def is_email_domain_allowed(self, email: Optional[str]) -> bool:
        ...
        try:
            ...
            return False
        allowed = [
            d.strip().lower() for d in (self.allowed_oauth_email_domains or []) if d
        ]
