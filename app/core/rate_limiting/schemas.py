...
class RateLimitMetadata(BaseModel):
    ...
    tpm_remaining: Optional[int] = Field(default=None, description="Tokens remaining in current minute")
    ...
    reset_time_tpm: Optional[datetime] = Field(default=None, description="When TPM counter resets")
...
class UsageStats(BaseModel):
    ...
    circuits: Dict[str, CircuitBreakerStatus] = Field(description="Per-bucket circuit breaker status")
    total_requests_today: int = Field(description="Total requests across all buckets today")
    total_requests_this_minute: int = Field(description="Total requests across all buckets this minute")
