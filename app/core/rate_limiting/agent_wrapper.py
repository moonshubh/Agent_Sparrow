...
def get_rate_limiter() -> BucketRateLimiter:
    ...
    """
    global _global_rate_limiter, _global_rate_limiter_pid
    current_pid = os.getpid()
    if _global_rate_limiter is None or _global_rate_limiter_pid != current_pid:
    ...
    def decorator(func: F) -> F:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.error(
                    f"Rate limiting service unavailable for {bucket}: {e.message}"
                )
                if fail_gracefully:
    ...
    def stream(self, *args, **kwargs) -> Any:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(
                    "Running event loop detected, skipping sync rate limit check"
                )
            ...
            if "loop" in locals():
                rate_check = loop.run_until_complete(
                    self.rate_limiter.check_and_consume(self.bucket)
                )
            else:
                rate_check = None
    ...
    async def astream(self, *args, **kwargs) -> Any:
        ...
        try:
            ...
            if hasattr(self.agent, "astream"):
                ...
                circuit_breaker = self.rate_limiter.get_circuit_breaker(self.bucket)
                return await circuit_breaker.call_sync(
                    self.agent.stream, *args, **kwargs
                )
            else:
                raise AttributeError(f"Agent {self.agent} does not have stream method")
...
def wrap_gemini_agent(
    agent: Any, bucket: str, model_name: str, **kwargs
) -> RateLimitedAgent:
    """
