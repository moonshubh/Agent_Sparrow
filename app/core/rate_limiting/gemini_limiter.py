...
class GeminiRateLimiter:
    ...
    def __init__(self, config: Optional[RateLimitConfig] = None):
        ...
        self.redis_client = redis.from_url(
            f"{self.config.redis_url}/{self.config.redis_db}", decode_responses=True
        )

        # Initialize Redis rate limiter
        self.redis_limiter = RedisRateLimiter(
            self.redis_client, self.config.redis_key_prefix
        )
    ...
    async def execute_with_protection(
        self, model: str, func: Callable, *args, **kwargs
    ) -> Any:
        """
        ...
        await self._await_rate_limit_slot(model)
        base_model = self.config.normalize_model_name(model)
    ...
    async def get_usage_stats(self) -> UsageStats:
        ...
        try:
            ...
            gemini_3_flash_circuit = await self.circuit_breakers[
                "gemini-3-flash"
            ].get_status()
            gemini_3_pro_circuit = await self.circuit_breakers[
                "gemini-3-pro"
            ].get_status()
            flash_circuit = await self.circuit_breakers["gemini-2.5-flash"].get_status()
            flash_lite_circuit = await self.circuit_breakers[
                "gemini-2.5-flash-lite"
            ].get_status()
            pro_circuit = await self.circuit_breakers["gemini-2.5-pro"].get_status()

            # Build metadata for each model
            gemini_3_flash_rpm_limit, gemini_3_flash_rpd_limit = (
                self.config.get_effective_limits("gemini-3-flash")
            )
            gemini_3_pro_rpm_limit, gemini_3_pro_rpd_limit = (
                self.config.get_effective_limits("gemini-3-pro")
            )
            flash_rpm_limit, flash_rpd_limit = self.config.get_effective_limits(
                "gemini-2.5-flash"
            )
            flash_lite_rpm_limit, flash_lite_rpd_limit = (
                self.config.get_effective_limits("gemini-2.5-flash-lite")
            )
            pro_rpm_limit, pro_rpd_limit = self.config.get_effective_limits(
                "gemini-2.5-pro"
            )
            ...
            def _build_metadata(
                ...
                return RateLimitMetadata(
                    ...
                    rpm_remaining=max(
                        0, effective_rpm_limit - stats_bucket.get("rpm", 0)
                    ),
                    rpd_limit=effective_rpd_limit,
                    rpd_used=stats_bucket.get("rpd", 0),
                    rpd_remaining=max(
                        0, effective_rpd_limit - stats_bucket.get("rpd", 0)
                    ),
                    reset_time_rpm=now.replace(second=0, microsecond=0)
                    + timedelta(minutes=1),
                    reset_time_rpd=now.replace(
            ...
            gemini_3_flash_metadata = _build_metadata(
                "gemini-3-flash",
                gemini_3_flash_rpm_limit,
                gemini_3_flash_rpd_limit,
                gemini_3_flash_stats,
            )
            gemini_3_pro_metadata = _build_metadata(
                "gemini-3-pro",
                gemini_3_pro_rpm_limit,
                gemini_3_pro_rpd_limit,
                gemini_3_pro_stats,
            )
            flash_metadata = _build_metadata(
                "gemini-2.5-flash", flash_rpm_limit, flash_rpd_limit, flash_stats
            )
            flash_lite_metadata = _build_metadata(
                "gemini-2.5-flash-lite",
                flash_lite_rpm_limit,
                flash_lite_rpd_limit,
                flash_lite_stats,
            )
            pro_metadata = _build_metadata(
                "gemini-2.5-pro", pro_rpm_limit, pro_rpd_limit, pro_stats
            )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health["rate_limits"] = {
                "gemini-3-flash": {
                    "rpm_utilization": stats.gemini_3_flash_stats.rpm_used
                    / max(1, stats.gemini_3_flash_stats.rpm_limit),
                    "rpd_utilization": stats.gemini_3_flash_stats.rpd_used
                    / max(1, stats.gemini_3_flash_stats.rpd_limit),
                },
                "gemini-3-pro": {
                    "rpm_utilization": stats.gemini_3_pro_stats.rpm_used
                    / max(1, stats.gemini_3_pro_stats.rpm_limit),
                    "rpd_utilization": stats.gemini_3_pro_stats.rpd_used
                    / max(1, stats.gemini_3_pro_stats.rpd_limit),
                },
                "gemini-2.5-flash": {
                    "rpm_utilization": stats.flash_stats.rpm_used
                    / max(1, stats.flash_stats.rpm_limit),
                    "rpd_utilization": stats.flash_stats.rpd_used
                    / max(1, stats.flash_stats.rpd_limit),
                },
                "gemini-2.5-flash-lite": {
                    "rpm_utilization": stats.flash_lite_stats.rpm_used
                    / max(1, stats.flash_lite_stats.rpm_limit),
                    "rpd_utilization": stats.flash_lite_stats.rpd_used
                    / max(1, stats.flash_lite_stats.rpd_limit),
                },
                "gemini-2.5-pro": {
                    "rpm_utilization": stats.pro_stats.rpm_used
                    / max(1, stats.pro_stats.rpm_limit),
                    "rpd_utilization": stats.pro_stats.rpd_used
                    / max(1, stats.pro_stats.rpd_limit),
                },
            }
