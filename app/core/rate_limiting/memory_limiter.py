...
class GeminiRateLimiter:
    ...
    async def check_limits(
        ...
        rpm_allowed, rpm_remaining, rpm_reset = await self._rpm_limiter.check_rate_limit(
            rpm_key, rpm_limit, 60, safety_margin
        )
        
        # Check per-day limit
        rpd_key = f"{user_id}:{model}:rpd"
        rpd_allowed, rpd_remaining, rpd_reset = await self._rpd_limiter.check_rate_limit(
        ...
        limit_info = {
            ...
            "limiting_factor": "rpm" if not rpm_allowed else ("rpd" if not rpd_allowed else None)
        }
