...
class InMemorySessionStore:
    ...
    async def store(
        self, session_id: str, session_type: str, data: Any, ttl: Optional[int] = None
    ) -> None:
        """
        ...
        async with self._lock:
            ...
            self._sessions[key] = SessionEntry(
                data=serialized_data, expires_at=expires_at
            )
    ...
    async def extend_ttl(
        self, session_id: str, session_type: str, additional_seconds: int
    ) -> bool:
        """
        ...
        async with self._lock:
            if key in self._sessions:
                ...
                entry.expires_at = max(entry.expires_at, datetime.utcnow()) + timedelta(
    ...
    def _find_oldest_key(self) -> Optional[str]:
        ...
        return min(self._sessions.keys(), key=lambda k: self._sessions[k].last_accessed)
    ...
    if _session_store is None:
        ...
        _session_store = InMemorySessionStore(
            ...
            cleanup_interval_seconds=getattr(
                settings, "session_store_cleanup_interval", 300
            ),
        )
...
async def store_session(
    session_id: str, session_type: str, state: Any, ttl: int = 3600
) -> None:
